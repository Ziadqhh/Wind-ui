--[[
    Deus Evolution UI | Version 1.3.7 (ESP & CORNER BOX UPDATE)
    Created by: Deus Mode (Gemini CLI)
    Style: Discord Client Aesthetic | Cloud Icons Linked | Fixed Sidebar Logic
]]
...
local DeusUI = {
    ...
    ESP = {
        Enabled = false,
        TargetPlayer = nil, -- If nil, apply to all
        CornerBoxes = true,
        Highlights = true,
        Names = true,
...
local function CreateESP(plr)
    ...
    -- Create Corner Box Lines (8 lines)
    ...
    connection = RunService.RenderStepped:Connect(function()
        ...
        if char and hrp and hum and hum.Health > 0 then
            ...
            if onScreen and DeusUI.ESP.CornerBoxes then
                ...
                -- Calculate 4 Corners
                local top = hrp.CFrame * CFrame.new(0, 3, 0)
                local bottom = hrp.CFrame * CFrame.new(0, -3.5, 0)
            ...
            if DeusUI.ESP.Highlights then
                if not Objects.Highlight then
                    Objects.Highlight = Instance.new("Highlight", (gethui and gethui()) or CoreGui)
                    Objects.Highlight.Adornee = char
                end
                Objects.Highlight.Enabled = true
                Objects.Highlight.FillColor = DeusUI.ESP.Color
            ...
            if DeusUI.ESP.Names then
                ...
                Objects.NameTag.Adornee = char
                Objects.NameTag.Enabled = true
                Objects.NameTag.TextLabel.Text = plr.DisplayName .. " [" .. math.floor(hum.Health) .. "]"
                Objects.NameTag.TextLabel.TextColor3 = DeusUI.ESP.Color
            elseif Objects.NameTag then
...
function DeusUI:CreateWindow(config)
    ...
    function WindowMethods:Settings() 
        ...
        SettingsTab:Dropdown({ Title = "Target Player", Values = function() 
            local p = Players:GetPlayers()
            table.insert(p, 1, "All Players")
            return p
        end, Default = "Choose Player...", Callback = function(p) 
            if p == "All Players" then DeusUI.ESP.TargetPlayer = nil else DeusUI.ESP.TargetPlayer = p end
        end })
