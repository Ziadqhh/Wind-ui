--[[
    Deus Evolution UI | Version 1.3.2 (DISCORD EDITION - ORDERED SECTIONS)
    Created by: Deus Mode (Gemini CLI)
    Style: Discord Client Aesthetic | Perfect Sidebar Organization
]]
...
local DeusUI = {
    ...
    MainFrame = nil,
    UIScale = nil,
    OrderCount = 0 -- عداد داخلي للترتيب
}
...
function DeusUI:CreateWindow(config)
    ...
    function Window:SidebarSection(title)
        ...
        local Sec = DeusUI:Create("TextLabel", {
            Name = "Section_" .. title, Parent = TabContainer, Size = UDim2.new(1, 0, 0, 22), BackgroundTransparency = 1, Text = title:upper(), TextColor3 = DeusUI.Themes[DeusUI.CurrentTheme].Accent, TextSize = 10, Font = Enum.Font.GothamBold, TextXAlignment = "Left", ZIndex = 4, LayoutOrder = self.OrderCount
    ...
    function Window:Tab(tabConfig)
        ...
        local TabBtn = DeusUI:Create("TextButton", { Parent = TabContainer, Size = UDim2.new(1, 0, 0, 30), BackgroundTransparency = 1, Text = "", AutoButtonColor = false, ZIndex = 4, LayoutOrder = self.OrderCount }, { 
    ...
    function Window:Settings()
        -- إنشاء سيكشن "OTHER" تلقائياً قبل زر الإعدادات لضمان الترتيب
