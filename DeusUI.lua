...
local DeusUI = {
    Themes = {
        ...
        Green = {
            Main = Color3.fromRGB(10, 35, 20),
            Secondary = Color3.fromRGB(15, 45, 30),
...
local NotifyGui = DeusUI:Create("ScreenGui", {
    Name = "DeusNotifications",
    Parent = (gethui and gethui()) or CoreGui
})

local NotifyHolder = DeusUI:Create("Frame", {
    Name = "Holder", Parent = NotifyGui,
    Size = UDim2.new(0, 250, 1, 0), Position = UDim2.new(1, -260, 0, 0),
    BackgroundTransparency = 1
}, { DeusUI:Create("UIListLayout", {VerticalAlignment = "Bottom", Padding = UDim.new(0, 10)}) })
...
function DeusUI:Notify(config)
    ...
    local Toast = self:Create("Frame", {
        Name = "Toast", Parent = NotifyHolder, Size = UDim2.new(1, 0, 0, 45),
        BackgroundColor3 = theme.Main, BackgroundTransparency = 0.2, Position = UDim2.new(1, 0, 0, 0)
    }, {
        self:Create("UICorner", {CornerRadius = UDim.new(0, 8)}),
        self:Create("UIStroke", {Color = theme.Outline, Transparency = 0.8}),
        self:Create("TextLabel", {
            Text = content, Size = UDim2.new(1, -20, 1, 0), Position = UDim2.new(0, 10, 0, 0),
            BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 13,
...
function DeusUI:CreateWindow(config)
    ...
    function Window:Tab(tabConfig)
        ...
        -- Helper: Element Lock (Fixed Layout & Input Blocking)
        local function applyLock(frame, locked, msg)
            if locked then
                local Lock = DeusUI:Create("Frame", {
                    Name = "LockOverlay", Parent = frame, Size = UDim2.new(1, 0, 1, 0), 
                    Position = UDim2.new(0, 0, 0, 0), BackgroundColor3 = Color3.new(0,0,0), 
                    BackgroundTransparency = 0.45, ZIndex = 100, -- أعلى شيء
                    Active = true -- يمنع ضغط ما خلفه
                }, { 
                    DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}),
                    DeusUI:Create("UIStroke", {Color = theme.Accent, Transparency = 0.5, Thickness = 1.5}),
                    DeusUI:Create("TextLabel", {
                        Text = "🔒 " .. (msg or "VIP ONLY"), Size = UDim2.new(1,0,1,0), 
                        BackgroundTransparency = 1, TextColor3 = Color3.new(1,1,1), 
                        Font = Enum.Font.GothamBold, TextSize = 13, ZIndex = 101
                    })
                })
                -- منع الضغط البرمجي أيضاً إذا أمكن
                if frame:FindFirstChild("Btn") then frame.Btn.Active = false end
                if frame:FindFirstChild("Switch") then frame.Switch.Active = false end
            end
        end
        ...
        function Tab:Slider(sConfig)
            ...
            local SliderFrame = DeusUI:Create("Frame", {
                Name = "SliderFrame", Parent = Page, Size = UDim2.new(0.95, 0, 0, 65), 
                BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4
            }, {
                DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}),
                DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}),
            ...
            local BarBack = DeusUI:Create("Frame", {
                Name = "BarBack", Parent = SliderFrame, Size = UDim2.new(1, 0, 0, 4), -- شريط نحيف نمط Material
                Position = UDim2.new(0, 0, 0, 40), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.6
            }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) })

            local BarFill = DeusUI:Create("Frame", {
                Name = "Fill", Parent = BarBack, Size = UDim2.new((val-min)/(max-min), 0, 1, 0), 
                BackgroundColor3 = theme.Accent
            }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) })

            local Knob = DeusUI:Create("Frame", {
                Name = "Knob", Parent = BarFill, Size = UDim2.new(0, 16, 0, 16), 
            ...
            local function update(input)
                ...
                DeusUI:Tween(BarFill, TweenInfo.new(0.1, Enum.EasingStyle.Quart), {Size = UDim2.new(sizeX, 0, 1, 0)})
                callback(val)
            end
            ...
            BarBack.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    dragging = true
                    update(input)
                    DeusUI:Tween(Knob, TweenInfo.new(0.2), {Size = UDim2.new(0, 20, 0, 20)}) -- تكبير المقبض عند السحب
                end
            end)
            ...
            UserInputService.InputEnded:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
                    dragging = false
                    DeusUI:Tween(Knob, TweenInfo.new(0.2), {Size = UDim2.new(0, 16, 0, 16)}) -- إعادة حجم المقبض
                end
            end)
    ...
    function Window:Settings()
        ...
        SettingsTab:Paragraph({
            ...
            Desc = "Customise your experience and manage the UI state."
        })

        SettingsTab:Dropdown({
            Title = "UI Theme",
            Values = {"Dark", "Light", "Purple", "Blue", "Green"},
            Default = "Dark",
            Callback = function(t)
                local newTheme = DeusUI.Themes[t]
                if newTheme then
                    DeusUI.CurrentTheme = t
                    DeusUI:Tween(MainFrame, TweenInfo.new(0.5), {BackgroundColor3 = newTheme.Main})
                    DeusUI:Tween(Sidebar, TweenInfo.new(0.5), {BackgroundColor3 = newTheme.Secondary})
                    Topbar.Title.TextColor3 = newTheme.Text
                    StatusBar.PerfText.TextColor3 = newTheme.TextSecondary
                    -- تحديث الألوان في كل المكتبة (يمكنك توسيع هذا المنطق ليشمل كافة الأزرار)
                    DeusUI:Notify({Content = "Theme changed to " .. t})
                end
            end
        })
        ...
        SettingsTab:Slider({
            ...
            Callback = function(val)
                local trans = val / 100
                DeusUI:Tween(MainFrame, TweenInfo.new(0.3), {BackgroundTransparency = trans})
                DeusUI:Tween(Sidebar, TweenInfo.new(0.3), {BackgroundTransparency = trans + 0.3})
                -- تحديث شفافية الإطارات لزيادة الجمالية
                MainFrame.UIStroke.Transparency = 0.85 + (trans * 0.1)
                Sidebar.UIStroke.Transparency = 0.9 + (trans * 0.05)
            end
        })
        ...
        SettingsTab:Button({
            ...
            Callback = function()
                DeusUI:Notify({Content = "Shutting down system..."})
                task.wait(0.5)
                ScreenGui:Destroy()
            end
        })
