...
local DeusUI = {
    Themes = {
        Dark = { Main = Color3.fromRGB(25, 25, 30), Secondary = Color3.fromRGB(32, 32, 40), ChannelBG = Color3.fromRGB(20, 20, 25), Outline = Color3.fromRGB(45, 45, 55), Text = Color3.fromRGB(255, 255, 255), TextSecondary = Color3.fromRGB(180, 180, 200), Accent = Color3.fromRGB(114, 137, 218), Success = Color3.fromRGB(67, 181, 129), Danger = Color3.fromRGB(240, 71, 71), Transparency = 0 },
        Midnight = { Main = Color3.fromRGB(10, 10, 15), Secondary = Color3.fromRGB(15, 15, 25), ChannelBG = Color3.fromRGB(5, 5, 10), Outline = Color3.fromRGB(30, 30, 45), Text = Color3.fromRGB(255, 255, 255), TextSecondary = Color3.fromRGB(150, 150, 180), Accent = Color3.fromRGB(0, 162, 255), Success = Color3.fromRGB(0, 255, 128), Danger = Color3.fromRGB(255, 50, 50), Transparency = 0 }
    },
    Icons = {},
...
function DeusUI:CreateWindow(config)
    ...
    local WindowSize = UDim2.new(0, 500, 0, 350); local MainFrame = self:Create("CanvasGroup", { Name = "MainFrame", Parent = ScreenGui, BackgroundColor3 = theme.Main, Position = UDim2.new(0.5, -250, 0.5, -175), Size = WindowSize, ClipsDescendants = true, ZIndex = 1, GroupTransparency = 0 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 12)}), self:Create("UIStroke", {Color = theme.Accent, Transparency = 0.4, Thickness = 2}), self:Create("UIScale", {Scale = 1}) }); self.MainFrame = MainFrame; self.UIScale = MainFrame.UIScale; table.insert(self.Elements, {Instance = MainFrame, Type = "MainFrame"})
    local Topbar = self:Create("Frame", { Name = "Topbar", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 40), Position = UDim2.new(0, 0, 0, 0), BackgroundColor3 = theme.Secondary, ZIndex = 3 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 12)}), self:Create("Frame", {Size = UDim2.new(1, 0, 0, 1), Position = UDim2.new(0, 0, 1, 0), BackgroundColor3 = theme.Outline, BackgroundTransparency = 0.8, ZIndex = 4}), self:Create("TextLabel", {Name = "Title", Text = config.Title or "DEUS EVOLUTION", Size = UDim2.new(0, 150, 1, 0), Position = UDim2.new(0, 15, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 13, Font = Enum.Font.GothamBold, TextXAlignment = "Left", ZIndex = 4}) }); table.insert(self.Elements, {Instance = Topbar, Type = "Topbar"})
    local Sidebar = self:Create("Frame", { Name = "Navbar", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 35), Position = UDim2.new(0, 0, 0, 40), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.5, ZIndex = 2 }); table.insert(self.Elements, {Instance = Sidebar, Type = "Sidebar"})
    local TabContainer = self:Create("ScrollingFrame", { Name = "TabScroll", Parent = Sidebar, Size = UDim2.new(1, -10, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1, ScrollBarThickness = 0, AutomaticCanvasSize = Enum.AutomaticSize.X, CanvasSize = UDim2.new(0,0,0,0), ZIndex = 3 }, { self:Create("UIListLayout", {FillDirection = "Horizontal", Padding = UDim.new(0, 8), VerticalAlignment = "Center", SortOrder = Enum.SortOrder.LayoutOrder}), self:Create("UIPadding", {PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(0, 10)}) }); local PageContainer = self:Create("Frame", { Name = "PageContainer", Parent = MainFrame, Size = UDim2.new(1, -20, 1, -85), Position = UDim2.new(0, 10, 0, 80), BackgroundTransparency = 1, ZIndex = 2 }); self.Topbar = Topbar
    ...
    function WindowMethods:SidebarSection(title) OrderTracker = OrderTracker + 1; local Sec = DeusUI:Create("Frame", { Name = "Section_" .. title, Parent = TabContainer, Size = UDim2.new(0, 1, 0, 15), BackgroundColor3 = DeusUI.Themes[DeusUI.CurrentTheme].Outline, BackgroundTransparency = 0.5, ZIndex = 4, LayoutOrder = OrderTracker }); return Sec end
    function WindowMethods:Tab(tabConfig)
        ...
        local TabBtn = DeusUI:Create("TextButton", { Parent = TabContainer, Size = UDim2.new(0, 0, 1, -6), AutomaticSize = "X", BackgroundTransparency = 1, Text = "", AutoButtonColor = false, ZIndex = 4, LayoutOrder = OrderTracker }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}), DeusUI:Create("Frame", { Name = "Indicator", Size = UDim2.new(0, 0, 0, 2), Position = UDim2.new(0.5, 0, 1, 2), AnchorPoint = Vector2.new(0.5, 1), BackgroundColor3 = theme.Accent, BackgroundTransparency = 1, ZIndex = 5 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }), DeusUI:Create("Frame", { Name = "Content", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, ZIndex = 5 }, { DeusUI:Create("UIListLayout", { FillDirection = "Horizontal", VerticalAlignment = "Center", Padding = UDim.new(0, 6), HorizontalAlignment = "Center" }), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12)}), tabIcon and DeusUI:Create("ImageLabel", { Name = "Icon", Size = UDim2.new(0, 14, 0, 14), BackgroundTransparency = 1, Image = tabIcon, ImageColor3 = theme.TextSecondary, ZIndex = 6 }) or nil, DeusUI:Create("TextLabel", { Name = "Label", Text = tabTitle, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = "X", BackgroundTransparency = 1, TextColor3 = theme.TextSecondary, TextSize = 11, Font = Enum.Font.GothamMedium, ZIndex = 6 }) }) })
        local tabData = {Type = "TabBtn", Instance = TabBtn, Selected = false}; table.insert(DeusUI.Elements, tabData)
        local function SetTabState(state) tabData.Selected = state; local cur = DeusUI.Themes[DeusUI.CurrentTheme]; local targetText = state and cur.Text or cur.TextSecondary; DeusUI:Tween(TabBtn.Content.Label, TweenInfo.new(0.2), {TextColor3 = targetText}); if TabBtn.Content:FindFirstChild("Icon") then DeusUI:Tween(TabBtn.Content.Icon, TweenInfo.new(0.2), {ImageColor3 = targetText}) end; if state then DeusUI:Tween(TabBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.9, BackgroundColor3 = cur.Accent}); DeusUI:Tween(TabBtn.Indicator, TweenInfo.new(0.2), {Size = UDim2.new(0.8, 0, 0, 2), BackgroundTransparency = 0, BackgroundColor3 = cur.Accent}) else DeusUI:Tween(TabBtn, TweenInfo.new(0.2), {BackgroundTransparency = 1}); DeusUI:Tween(TabBtn.Indicator, TweenInfo.new(0.2), {Size = UDim2.new(0, 0, 0, 2), BackgroundTransparency = 1}) end end
