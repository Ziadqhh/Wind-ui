--[[
    Deus Evolution UI | Version 1.3.2 (SCROLLING FIXED - STABLE)
    Created by: Deus Mode (Gemini CLI)
    Style: Minimal Floating Card | Optimized Scrolling
]]
...
-- Notification System (Original Stable)
...
function DeusUI:CreateWindow(config)
    ...
    function Window:Tab(tabConfig)
        ...
        local Page = DeusUI:Create("ScrollingFrame", { 
            ...
            DeusUI:Create("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 10)})
        })
        
        local TabBtn = DeusUI:Create("TextButton", { 
            Name = tabTitle .. "Btn", Parent = TabContainer, Size = UDim2.new(1, -10, 0, 32), 
            ...
            DeusUI:Create("Frame", { Name = "Indicator", Size = UDim2.new(0, 3, 0, 0), Position = UDim2.new(0, 2, 0.5, 0), AnchorPoint = Vector2.new(0, 0.5), BackgroundColor3 = theme.Accent, BackgroundTransparency = 1, ZIndex = 5 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }),
        ...
        local function SetTabState(state)
            ...
            DeusUI:Tween(TabBtn.Indicator, TweenInfo.new(0.3), {Size = state and UDim2.new(0, 3, 0.6, 0) or UDim2.new(0, 3, 0, 0), BackgroundTransparency = state and 0 or 1})
        end
        ...
        TabBtn.MouseButton1Click:Connect(function() 
            ...
            for _, el in pairs(DeusUI.Elements) do 
                if el.Type == "TabBtn" then 
                    ...
                    DeusUI:Tween(el.Instance.Indicator, TweenInfo.new(0.3), {Size = UDim2.new(0, 3, 0, 0), BackgroundTransparency = 1})
                end 
            end
        ...
        function Tab:Slider(sConfig) 
            ...
            local Bar = DeusUI:Create("Frame", {Parent = SliderFrame, Size = UDim2.new(1, 0, 0, 4), Position = UDim2.new(0, 0, 0, 38), BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 0.6, ZIndex = 5}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) })
