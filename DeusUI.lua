...
function DeusUI:CreateWindow(config)
    ...
    function Window:Tab(tabCfg)
        ...
        local Page = self:Create("ScrollingFrame", { 
            ...
            self:Create("UIListLayout", {Padding = UDim.new(0, 6)}) 
        })
        ...
        local TabBtn = self:Create("TextButton", {
            ...
            self:Create("UIListLayout", {FillDirection = "Horizontal", VerticalAlignment = "Center", Padding = UDim.new(0, 6), HorizontalAlignment = "Center"}),
            icon and self:Create("ImageLabel", {Size = UDim2.new(0, 15, 0, 15), BackgroundTransparency = 1, Image = icon, ImageColor3 = theme.TextSecondary}) or nil,
            self:Create("TextLabel", {Name = "Label", Text = title, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = "X", BackgroundTransparency = 1, TextColor3 = theme.TextSecondary, TextSize = 12, Font = Enum.Font.GothamMedium})
        })
        ...
        local function lock(frame, locked, msg)
            if locked then
                self:Create("Frame", {
                    ...
                    self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}),
                    self:Create("TextLabel", {Text = "🔒 " .. (msg or "VIP"), Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold, TextSize = 11})
                })
            end
        end
        ...
        function Tab:Button(cfg)
            local f = self:Create("Frame", { 
                ...
                self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
                self:Stroke(nil, theme.StrokeColor, 1.2, 0.35),
                self:Create("TextButton", {Name = "Btn", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = cfg.Title or "Button", TextColor3 = theme.Text, TextSize = 12, Font = Enum.Font.GothamBold})
            })
        ...
        function Tab:Toggle(cfg)
            ...
            local f = self:Create("Frame", { 
                ...
                self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
                self:Stroke(nil, theme.StrokeColor, 1.2, 0.35),
                self:Create("TextLabel", {Text = cfg.Title or "Toggle", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 12, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -55, 1, 0), Position = UDim2.new(0, 10, 0, 0), TextXAlignment = "Left"})
            })
            local sw = self:Create("TextButton", { 
                Parent = f, 
            ...
            sw.MouseButton1Click:Connect(function()
                on = not on
                self:Tween(cir, TweenInfo.new(0.2), {Position = on and UDim2.new(1, -2, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), AnchorPoint = on and Vector2.new(1, 0.5) or Vector2.new(0, 0.5)})
                self:Tween(sw, TweenInfo.new(0.2), {BackgroundColor3 = on and theme.Accent or Color3.fromRGB(60, 60, 70)})
                cfg.Callback(on)
            end)
        ...
        function Tab:Slider(cfg)
            ...
            local f = self:Create("Frame", { 
                ...
                self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
                self:Stroke(nil, theme.StrokeColor, 1.2, 0.35),
                self:Create("TextLabel", {Name = "Title", Text = cfg.Title or "Slider", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 12, Font = Enum.Font.GothamMedium, Size = UDim2.new(0.65, 0, 0, 18), Position = UDim2.new(0, 10, 0, 6), TextXAlignment = "Left"}),
                self:Create("TextLabel", {Name = "Val", Text = tostring(val), BackgroundTransparency = 1, TextColor3 = theme.Accent, TextSize = 12, Font = Enum.Font.GothamBold, Size = UDim2.new(0.35, 0, 0, 18), Position = UDim2.new(0.65, 0, 0, 6), TextXAlignment = "Right"})
            })
            local bar = self:Create("Frame", {Parent = f, Size = UDim2.new(1, -16, 0, 4), Position = UDim2.new(0, 8, 0, 32), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.5}, {self:Create("UICorner", {CornerRadius = UDim.new(1, 0)})})
            local fill = self:Create("Frame", {Parent = bar, Size = UDim2.new((val-mn)/(mx-mn), 0, 1, 0), BackgroundColor3 = theme.Accent}, {self:Create("UICorner", {CornerRadius = UDim.new(1, 0)})})
            local knob = self:Create("Frame", {Parent = fill, Size = UDim2.new(0, 14, 0, 14), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1)}, {self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), self:Stroke(nil, theme.Accent, 1.5, 0)})
        ...
        function Tab:Dropdown(cfg)
            ...
            local hdr = self:Create("TextButton", {Parent = f, Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, Text = ""})
            self:Create("TextLabel", {Parent = hdr, Text = cfg.Title or "Dropdown", Size = UDim2.new(0.4, 0, 1, 0), Position = UDim2.new(0, 10, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.Text, Font = Enum.Font.GothamMedium, TextSize = 12, TextXAlignment = "Left"})
            local cont = self:Create("TextLabel", {Parent = hdr, Text = sel, Size = UDim2.new(0.45, 0, 1, 0), Position = UDim2.new(0.3, 0, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.TextSecondary, Font = Enum.Font.GothamMedium, TextSize = 11, TextXAlignment = "Right"})
            local arr = self:Create("TextLabel", {Parent = hdr, Text = "▼", Size = UDim2.new(0.15, 0, 1, 0), Position = UDim2.new(0.85, 0, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.Accent, Font = Enum.Font.GothamBold, TextSize = 9, TextXAlignment = "Center"})
            local items = self:Create("Frame", {Parent = f, Size = UDim2.new(1, 0, 0, 0), Position = UDim2.new(0, 0, 0, 30), BackgroundTransparency = 1, Visible = false}, {self:Create("UIListLayout", {Padding = UDim.new(0, 3)})})
            for _, v in pairs(vals) do
                local ib = self:Create("TextButton", {Parent = items, Size = UDim2.new(1, 0, 0, 24), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.2, Text = "  " .. v, TextColor3 = theme.Text, Font = Enum.Font.GothamMedium, TextSize = 11, TextXAlignment = "Left"}, {self:Create("UICorner", {CornerRadius = UDim.new(0, 4)})})
                ib.MouseButton1Click:Connect(function() sel = v; cont.Text = sel; open = false; f.Size = UDim2.new(0.94, 0, 0, 36); items.Visible = false; self:Tween(arr, TweenInfo.new(0.2), {Rotation = 0}); cfg.Callback(sel) end)
            end
            hdr.MouseButton1Click:Connect(function()
                open = not open
                if open then f.Size = UDim2.new(0.94, 0, 0, 36 + (#vals * 27)); items.Visible = true; self:Tween(arr, TweenInfo.new(0.2), {Rotation = 180})
                else f.Size = UDim2.new(0.94, 0, 0, 36); items.Visible = false; self:Tween(arr, TweenInfo.new(0.2), {Rotation = 0}) end
            end)
        ...
        function Tab:Keybind(cfg)
            ...
            local f = self:Create("Frame", { 
                ...
                self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
                self:Stroke(nil, theme.StrokeColor, 1.2, 0.35),
                self:Create("TextLabel", {Text = cfg.Title or "Keybind", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 12, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -80, 1, 0), Position = UDim2.new(0, 10, 0, 0), TextXAlignment = "Left"})
            })
            local btn = self:Create("TextButton", {Parent = f, Size = UDim2.new(0, 65, 0, 24), Position = UDim2.new(1, -8, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.3, Text = key.Name, TextColor3 = theme.Accent, Font = Enum.Font.GothamBold, TextSize = 11}, {self:Create("UICorner", {CornerRadius = UDim.new(0, 5)})})
        ...
        function Tab:Colorpicker(cfg)
            ...
            local f = self:Create("Frame", { 
                ...
                self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
                self:Stroke(nil, theme.StrokeColor, 1.2, 0.35),
                self:Create("TextLabel", {Text = cfg.Title or "Color", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 12, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -55, 1, 0), Position = UDim2.new(0, 10, 0, 0), TextXAlignment = "Left"})
            })
            local view = self:Create("TextButton", {Parent = f, Size = UDim2.new(0, 38, 0, 22), Position = UDim2.new(1, -8, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = clr, Text = ""}, {self:Create("UICorner", {CornerRadius = UDim.new(0, 5)})})
        ...
        function Tab:Paragraph(cfg)
            local f = self:Create("Frame", { 
                ...
                self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
                self:Stroke(nil, theme.StrokeColor, 1.2, 0.35),
                self:Create("UIPadding", {PaddingTop = UDim.new(0, 8), PaddingBottom = UDim.new(0, 8), PaddingLeft = UDim.new(0, 10), PaddingRight = UDim.new(0, 10)}),
                self:Create("UIListLayout", {Padding = UDim.new(0, 3)}),
                self:Create("TextLabel", {Text = cfg.Title or "Info", BackgroundTransparency = 1, TextColor3 = theme.Accent, Font = Enum.Font.GothamBold, TextSize = 12, AutomaticSize = "XY"}),
                self:Create("TextLabel", {Text = cfg.Desc or "", BackgroundTransparency = 1, TextColor3 = theme.TextSecondary, Font = Enum.Font.Gotham, TextSize = 11, AutomaticSize = "Y", Size = UDim2.new(1, 0, 0, 0), TextWrapped = true, TextXAlignment = "Left"})
            })
