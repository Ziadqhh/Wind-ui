--[[ 
    Deus Evolution UI | Version 1.2.6 (WIND-UI CLONED COLORPICKER)
    Created by: Deus Mode (Gemini CLI)
    Style: Full Dynamic Glassmorphism | Professional Color Systems
]]
...
-- Advanced ColorPicker (Wind-UI Style Clone)
function DeusUI:OpenColorPicker(defaultColor, callback)
    ...
    local PickerFrame = self:Create("Frame", { Name = "PickerFrame", Parent = Overlay, Size = UDim2.new(0, 320, 0, 380), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.05, Active = true }, {
    ...
    local SatVibMap = self:Create("ImageLabel", { Name = "SatVibMap", Parent = PickerFrame, Size = UDim2.new(0, 200, 0, 200), Position = UDim2.new(0, 20, 0, 20), Image = "rbxassetid://4155801252", BackgroundColor3 = Color3.fromHSV(h, 1, 1) }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 8)}) })
    local MapKnob = self:Create("Frame", { Name = "MapKnob", Parent = SatVibMap, Size = UDim2.new(0, 12, 0, 12), Position = UDim2.new(s, 0, 1-v, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1) }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), self:Create("UIStroke", {Thickness = 1.5}) })

    -- Hue Bar (Rainbow)
    local HueBar = self:Create("Frame", { Name = "HueBar", Parent = PickerFrame, Size = UDim2.new(0, 25, 0, 200), Position = UDim2.new(0, 235, 0, 20) }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) })
    ...
    local HueKnob = self:Create("Frame", { Name = "HueKnob", Parent = HueBar, Size = UDim2.new(1, 4, 0, 4), Position = UDim2.new(0.5, 0, h, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1) }, { self:Create("UIStroke", {Thickness = 1.5}) })

    -- Inputs (Hex/RGB)
    local Preview = self:Create("Frame", { Name = "Preview", Parent = PickerFrame, Size = UDim2.new(0, 40, 0, 40), Position = UDim2.new(0, 20, 0, 235), BackgroundColor3 = selectedColor }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
    local HexBox = self:Create("TextBox", { Name = "Hex", Parent = PickerFrame, Size = UDim2.new(0, 100, 0, 30), Position = UDim2.new(0, 75, 0, 240), BackgroundColor3 = theme.Secondary, Text = "#" .. selectedColor:ToHex(), TextColor3 = theme.Text, Font = Enum.Font.GothamMedium, TextSize = 12 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
    ...
    local ApplyBtn = self:Create("TextButton", { Parent = PickerFrame, Size = UDim2.new(1, -40, 0, 35), Position = UDim2.new(0.5, 0, 1, -20), AnchorPoint = Vector2.new(0.5, 1), BackgroundColor3 = theme.Accent, Text = "Apply Color", TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold, TextSize = 14 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
...
function DeusUI:CreateWindow(config)
    ...
    function Window:Tab(tabConfig)
        ...
        function Tab:Button(btnConfig)
            local ButtonFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 40), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.3 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.85, Thickness = 1.2}),
            ...
            ButtonFrame.Btn.MouseButton1Click:Connect(function() btnConfig.Callback() end); table.insert(DeusUI.Elements, {Type = "Button", Instance = ButtonFrame})
        end
        ...
        function Tab:Dropdown(dConfig)
            ...
            table.insert(DeusUI.Elements, {Type = "Text", Instance = Title}); table.insert(DeusUI.Elements, {Type = "AccentText", Instance = SelText})
        end
    ...
    task.spawn(function() while MainFrame.Parent do local fps = math.floor(workspace:GetRealPhysicsFPS()); local ping = math.floor(LocalPlayer:GetNetworkPing() * 1000); StatusBar.PerfText.Text = string.format("FPS: %d | Ping: %dms | Deus Evolution v1.2.5", fps, ping); task.wait(1) end end)
    return Window
end
