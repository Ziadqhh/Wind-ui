--[[ 
    Deus Evolution UI | Version 1.2.5 (MODAL & STABILITY FIX)
...
-- Improved ColorPicker Popup
function DeusUI:OpenColorPicker(defaultColor, callback)
    local theme = self.Themes[self.CurrentTheme]
    local selectedColor = defaultColor or Color3.new(1,1,1)
    
    local Overlay = self:Create("TextButton", { Name = "PickerOverlay", Parent = self.ScreenGui, Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 1, ZIndex = 1000, Text = "", AutoButtonColor = false })
    local PickerFrame = self:Create("Frame", { Name = "PickerFrame", Parent = Overlay, Size = UDim2.new(0, 260, 0, 300), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.05, Active = true }, {
    ...
    local Wheel = self:Create("ImageLabel", { Name = "Wheel", Parent = PickerFrame, Size = UDim2.new(0, 160, 0, 160), Position = UDim2.new(0.5, 0, 0, 20), AnchorPoint = Vector2.new(0.5, 0), Image = "rbxassetid://2849458409", BackgroundTransparency = 1, ZIndex = 1001 })
    local Knob = self:Create("Frame", { Name = "Knob", Parent = Wheel, Size = UDim2.new(0, 12, 0, 12), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1), ZIndex = 1002, Position = UDim2.new(0.5, 0, 0.5, 0) }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), self:Create("UIStroke", {Thickness = 1.5}) })
    local Preview = self:Create("Frame", { Name = "Preview", Parent = PickerFrame, Size = UDim2.new(0, 60, 0, 30), Position = UDim2.new(0.5, 0, 0, 195), AnchorPoint = Vector2.new(0.5, 0), BackgroundColor3 = selectedColor, ZIndex = 1001 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), self:Create("UIStroke", {Transparency = 0.8}) })
    ...
    local function updateColor(input)
        ...
        local s = dist / radius
        selectedColor = Color3.fromHSV(h, s, 1)
        
        local moveDist = math.clamp(offset.Magnitude, 0, radius)
        local moveX = math.cos(angle) * moveDist
        local moveY = math.sin(angle) * moveDist
        
        Knob.Position = UDim2.new(0.5, moveX, 0.5, moveY)
    ...
    local Buttons = self:Create("Frame", { Parent = PickerFrame, Size = UDim2.new(1, -40, 0, 35), Position = UDim2.new(0.5, 0, 1, -15), AnchorPoint = Vector2.new(0.5, 1), BackgroundTransparency = 1, ZIndex = 1001 }, { self:Create("UIListLayout", {FillDirection = "Horizontal", Padding = UDim.new(0, 10), HorizontalAlignment = "Center"}) })
    local function createBtn(txt, clr, f)
        local b = self:Create("TextButton", { Parent = Buttons, Size = UDim2.new(0.45, 0, 1, 0), BackgroundColor3 = clr, Text = txt, TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold, TextSize = 14, ZIndex = 1002 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
        b.MouseButton1Click:Connect(f)
    end
    createBtn("Cancel", theme.Secondary, function() Overlay:Destroy() end)
    createBtn("Apply", theme.Accent, function() callback(selectedColor); Overlay:Destroy() end)
end
...
function DeusUI:CreateWindow(config)
    ...
    local function updateGlobalStyles()
        ...
        Topbar.Title.TextColor3 = current.Text; Topbar.Separator.BackgroundColor3 = current.Outline; StatusBar.PerfText.TextColor3 = current.TextSecondary; MinBtn.BackgroundColor3 = current.Secondary; MinBtn.TextColor3 = current.Text
    ...
    function Window:Tab(tabConfig)
        ...
        function Tab:Button(btnConfig)
            local ButtonFrame = DeusUI:Create("Frame", { Name = "ButtonFrame", Parent = Page, Size = UDim2.new(0.95, 0, 0, 40), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.3 }, {
                DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.85, Thickness = 1.2}),
        ...
        function Tab:Colorpicker(cConfig)
            ...
            local ColorFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}),
                DeusUI:Create("TextLabel", {Name = "T", Text = cConfig.Title or "Color", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -60, 1, 0), TextXAlignment = "Left"})
            })
            local ColorView = DeusUI:Create("TextButton", { Name = "View", Parent = ColorFrame, Size = UDim2.new(0, 40, 0, 22), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = current, Text = "" }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) })
    ...
    task.spawn(function()
        while MainFrame.Parent do
            local fps = math.floor(workspace:GetRealPhysicsFPS()); local ping = math.floor(LocalPlayer:GetNetworkPing() * 1000); StatusBar.PerfText.Text = string.format("FPS: %d | Ping: %dms | Deus Evolution v1.2.5", fps, ping); task.wait(1)
        end
    end)
    return Window
end
