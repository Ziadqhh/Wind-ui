--[[
    Deus Evolution UI | Version 1.3.0 (STABILITY FIX)
...
function DeusUI:CreateWindow(config)
    ...
    local MinBtn = self:Create("TextButton", { Name = "Min", Parent = Controls, Size = UDim2.new(0, 26, 0, 26), BackgroundColor3 = theme.Secondary, Text = "-", TextColor3 = theme.Text, TextSize = 18, Font = Enum.Font.GothamBold, ZIndex = 101 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), self:Create("UIStroke", {Color = theme.Outline, Transparency = 0.8}) })
    local CloseBtn = self:Create("TextButton", { Name = "Close", Parent = Controls, Size = UDim2.new(0, 26, 0, 26), BackgroundColor3 = Color3.fromRGB(180, 40, 40), Text = "×", TextColor3 = Color3.new(1, 1, 1), TextSize = 18, Font = Enum.Font.GothamBold, ZIndex = 101 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
    ...
    task.spawn(function() 
        while MainFrame.Parent do 
            ...
            StatusBar.PerfText.Text = string.format("FPS: %d | Ping: %dms | Deus Evolution v1.3.0", fps, ping)
            task.wait(1) 
        end 
    end)
