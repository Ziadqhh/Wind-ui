--[[ 
    Deus Evolution UI | Version 1.2.4 (PREMIUM TOGGLE & FIXES)
...
-- Professional ColorPicker Popup
function DeusUI:OpenColorPicker(defaultColor, callback)
    local theme = self.Themes[self.CurrentTheme]
    local selectedColor = defaultColor or Color3.new(1,1,1)
    local Overlay = self:Create("Frame", { Name = "PickerOverlay", Parent = self.ScreenGui, Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 1, ZIndex = 500, Active = true })
    local PickerFrame = self:Create("Frame", { Name = "PickerFrame", Parent = Overlay, Size = UDim2.new(0, 280, 0, 320), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.1 }, {
    ...
    self:Tween(PickerFrame, TweenInfo.new(0.4, Enum.EasingStyle.Back), {Size = UDim2.new(0, 280, 0, 320)})
    local Wheel = self:Create("ImageLabel", { Name = "Wheel", Parent = PickerFrame, Size = UDim2.new(0, 180, 0, 180), Position = UDim2.new(0.5, 0, 0, 30), AnchorPoint = Vector2.new(0.5, 0), Image = "rbxassetid://6034289542", BackgroundTransparency = 1 })
    local Knob = self:Create("Frame", { Name = "Knob", Parent = Wheel, Size = UDim2.new(0, 14, 0, 14), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1) }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), self:Create("UIStroke", {Thickness = 2}) })
    local Preview = self:Create("Frame", { Name = "Preview", Parent = PickerFrame, Size = UDim2.new(0, 40, 0, 40), Position = UDim2.new(0.5, 0, 0, 220), AnchorPoint = Vector2.new(0.5, 0), BackgroundColor3 = selectedColor }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 8)}) })
    local function updateColor(input)
        local center = Wheel.AbsolutePosition + (Wheel.AbsoluteSize / 2); local offset = Vector2.new(input.Position.X - center.X, input.Position.Y - center.Y)
        local radius = Wheel.AbsoluteSize.X / 2; local dist = math.clamp(offset.Magnitude, 0, radius); local angle = math.atan2(offset.Y, offset.X)
        local h = (math.deg(angle) + 180) / 360; local s = dist / radius; selectedColor = Color3.fromHSV(h, s, 1)
        Knob.Position = UDim2.new(0.5, offset.X * (dist/offset.Magnitude), 0.5, offset.Y * (dist/offset.Magnitude)); Preview.BackgroundColor3 = selectedColor
    end
    local wheelDragging = false
    Wheel.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then wheelDragging = true; updateColor(input) end end)
    UserInputService.InputChanged:Connect(function(input) if wheelDragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then updateColor(input) end end)
    UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then wheelDragging = false end end)
    local Buttons = self:Create("Frame", { Name = "Buttons", Parent = PickerFrame, Size = UDim2.new(1, -40, 0, 35), Position = UDim2.new(0.5, 0, 1, -20), AnchorPoint = Vector2.new(0.5, 1), BackgroundTransparency = 1 }, { self:Create("UIListLayout", {FillDirection = "Horizontal", Padding = UDim.new(0, 10), HorizontalAlignment = "Center"}) })
    local function createPickerBtn(text, color, func)
        local b = self:Create("TextButton", { Parent = Buttons, Size = UDim2.new(0.45, 0, 1, 0), BackgroundColor3 = color, Text = text, TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold, TextSize = 14 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
        b.MouseButton1Click:Connect(func)
    end
    createPickerBtn("Cancel", theme.Secondary, function() Overlay:Destroy() end)
    createPickerBtn("Apply", theme.Accent, function() callback(selectedColor); Overlay:Destroy() end)
end
...
function DeusUI:CreateWindow(config)
    ...
    function Window:Tab(tabConfig)
        ...
        local function applyLock(frame, locked, msg)
            if locked then local Lock = DeusUI:Create("Frame", { Name = "Lock", Parent = frame, Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 0.5, ZIndex = 100, Active = true }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("TextLabel", {Text = "🔒 " .. (msg or "VIP"), Size = UDim2.new(1,0,1,0), BackgroundTransparency = 1, TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold, TextSize = 12, ZIndex = 101}) }) end
        end

        function Tab:Button(btnConfig)
            local ButtonFrame = DeusUI:Create("Frame", { Name = "ButtonFrame", Parent = Page, Size = UDim2.new(0.95, 0, 0, 40), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.3 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.85, Thickness = 1.2}), DeusUI:Create("TextButton", {Name = "Btn", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = btnConfig.Title or "Button", TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamBold}) })
            ButtonFrame.Btn.MouseButton1Click:Connect(function() btnConfig.Callback() end); table.insert(DeusUI.Elements, {Type = "Button", Instance = ButtonFrame}); applyLock(ButtonFrame, btnConfig.Locked, btnConfig.LockMessage)
        end

        function Tab:Toggle(tConfig)
            local toggled = tConfig.Default or false; local currentTheme = DeusUI.Themes[DeusUI.CurrentTheme]
            local ToggleFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = tConfig.Title or "Toggle", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -60, 1, 0), TextXAlignment = "Left"}) })
            local Switch = DeusUI:Create("TextButton", { Name = "Switch", Parent = ToggleFrame, Size = UDim2.new(0, 42, 0, 22), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = toggled and currentTheme.Accent or Color3.fromRGB(60, 60, 60), Text = "", AutoButtonColor = false }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), DeusUI:Create("UIStroke", {Name = "S", Color = Color3.new(1,1,1), Transparency = 0.8, Thickness = 1.2}) })
            local Circle = DeusUI:Create("Frame", { Name = "Circle", Parent = Switch, Size = UDim2.new(0, 18, 0, 18), Position = toggled and UDim2.new(1, -2, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), AnchorPoint = toggled and Vector2.new(1, 0.5) or Vector2.new(0, 0.5), BackgroundColor3 = Color3.new(1, 1, 1) }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), DeusUI:Create("UIStroke", {Color = Color3.new(0,0,0), Transparency = 0.9, Thickness = 1}) })
            Switch.MouseButton1Click:Connect(function() toggled = not toggled; local targetPos = toggled and UDim2.new(1, -2, 0.5, 0) or UDim2.new(0, 2, 0.5, 0); local targetAnchor = toggled and Vector2.new(1, 0.5) or Vector2.new(0, 0.5); local targetColor = toggled and DeusUI.Themes[DeusUI.CurrentTheme].Accent or Color3.fromRGB(60, 60, 60); DeusUI:Tween(Circle, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {Position = targetPos, AnchorPoint = targetAnchor}); DeusUI:Tween(Switch, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {BackgroundColor3 = targetColor}); tConfig.Callback(toggled) end)
        ...
        function Tab:Dropdown(dConfig)
            ...
            for _, v in pairs(dValues) do local Opt = DeusUI:Create("TextButton", {Parent = OptionHolder, Size = UDim2.new(1, 0, 0, 28), BackgroundColor3 = theme.Main, Text = v, TextColor3 = theme.TextSecondary, Font = Enum.Font.Gotham, TextSize = 13}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) }); Opt.MouseButton1Click:Connect(function() selected = v; SelText.Text = v; opened = false; DeusUI:Tween(DropdownFrame, TweenInfo.new(0.3), {Size = UDim2.new(0.95, 0, 0, 45)}); dConfig.Callback(v) end) end
        ...
        function Tab:Keybind(kConfig)
            ...
            local BindFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = kConfig.Title or "Keybind", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -100, 1, 0), TextXAlignment = "Left"}) })
        ...
        function Tab:Colorpicker(cConfig)
            ...
            local ColorFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = cConfig.Title or "Color", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -60, 1, 0), TextXAlignment = "Left"}) })
            local ColorView = DeusUI:Create("TextButton", { Parent = ColorFrame, Size = UDim2.new(0, 40, 0, 22), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = current, Text = "" }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) })
    ...
    task.spawn(function()
        while MainFrame.Parent do
            local fps = math.floor(workspace:GetRealPhysicsFPS()); local ping = math.floor(LocalPlayer:GetNetworkPing() * 1000); StatusBar.PerfText.Text = string.format("FPS: %d | Ping: %dms | Deus Evolution v1.2.4", fps, ping); task.wait(1)
        end
    end)
    return Window
end
