--[[ 
    Deus Evolution UI | Version 1.2.7 (CONTROL & PICKER FIX)
    Created by: Deus Mode (Gemini CLI)
    Style: Full Dynamic Glassmorphism | Mobile Optimized
]]
...
-- Fixed ColorPicker Popup
function DeusUI:OpenColorPicker(defaultColor, callback)
    local theme = self.Themes[self.CurrentTheme]
    local h, s, v = Color3.toHSV(defaultColor)
    local selectedColor = defaultColor
    
    local Overlay = self:Create("TextButton", { Name = "PickerOverlay", Parent = self.ScreenGui, Size = UDim2.new(1, 0, 1, 0), BackgroundColor3 = Color3.new(0,0,0), BackgroundTransparency = 1, ZIndex = 2000, Text = "", AutoButtonColor = false })
    local PickerFrame = self:Create("Frame", { Name = "PickerFrame", Parent = Overlay, Size = UDim2.new(0, 300, 0, 350), Position = UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.05, Active = true, ZIndex = 2001 }, {
        self:Create("UICorner", {CornerRadius = UDim.new(0, 12)}),
        self:Create("UIStroke", {Color = theme.Outline, Transparency = 0.7, Thickness = 1.5})
    })

    self:Tween(Overlay, TweenInfo.new(0.3), {BackgroundTransparency = 0.5})
    Overlay.MouseButton1Click:Connect(function() Overlay:Destroy() end)

    local SatVibMap = self:Create("ImageLabel", { Name = "SatVibMap", Parent = PickerFrame, Size = UDim2.new(0, 180, 0, 180), Position = UDim2.new(0, 20, 0, 20), Image = "rbxassetid://4155801252", BackgroundColor3 = Color3.fromHSV(h, 1, 1), ZIndex = 2002 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 8)}) })
    local MapKnob = self:Create("Frame", { Name = "MapKnob", Parent = SatVibMap, Size = UDim2.new(0, 12, 0, 12), Position = UDim2.new(s, 0, 1-v, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1), ZIndex = 2003 }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), self:Create("UIStroke", {Thickness = 1.5}) })

    local HueBar = self:Create("Frame", { Name = "HueBar", Parent = PickerFrame, Size = UDim2.new(0, 25, 0, 180), Position = UDim2.new(0, 220, 0, 20), ZIndex = 2002 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) })
    ...
    local HueKnob = self:Create("Frame", { Name = "HueKnob", Parent = HueBar, Size = UDim2.new(1, 4, 0, 4), Position = UDim2.new(0.5, 0, h, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1,1,1), ZIndex = 2003 }, { self:Create("UIStroke", {Thickness = 1.5}) })

    local Preview = self:Create("Frame", { Name = "Preview", Parent = PickerFrame, Size = UDim2.new(0, 50, 0, 50), Position = UDim2.new(0, 20, 0, 220), BackgroundColor3 = selectedColor, ZIndex = 2002 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 8)}) })
    local HexBox = self:Create("TextBox", { Name = "Hex", Parent = PickerFrame, Size = UDim2.new(0, 120, 0, 35), Position = UDim2.new(0, 85, 0, 227), BackgroundColor3 = theme.Secondary, Text = "#" .. selectedColor:ToHex(), TextColor3 = theme.Text, Font = Enum.Font.GothamMedium, TextSize = 14, ZIndex = 2002 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })

    local function update()
        selectedColor = Color3.fromHSV(h, s, v); SatVibMap.BackgroundColor3 = Color3.fromHSV(h, 1, 1); Preview.BackgroundColor3 = selectedColor; HexBox.Text = "#" .. selectedColor:ToHex(); MapKnob.Position = UDim2.new(s, 0, 1-v, 0); HueKnob.Position = UDim2.new(0.5, 0, h, 0)
    end
    ...
    UserInputService.InputChanged:Connect(function(input)
        if (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            if mapDragging then
                ...
                s = math.clamp(pos.X / SatVibMap.AbsoluteSize.X, 0, 1); v = 1 - math.clamp(pos.Y / SatVibMap.AbsoluteSize.Y, 0, 1); update()
            elseif hueDragging then
                local pos = input.Position.Y - HueBar.AbsolutePosition.Y
                h = math.clamp(pos / HueBar.AbsoluteSize.Y, 0, 1); update()
            end
        end
    end)
    ...
    local ApplyBtn = self:Create("TextButton", { Parent = PickerFrame, Size = UDim2.new(1, -40, 0, 40), Position = UDim2.new(0.5, 0, 1, -15), AnchorPoint = Vector2.new(0.5, 1), BackgroundColor3 = theme.Accent, Text = "Apply Color", TextColor3 = Color3.new(1,1,1), Font = Enum.Font.GothamBold, TextSize = 14, ZIndex = 2002 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
    ApplyBtn.MouseButton1Click:Connect(function() callback(selectedColor); Overlay:Destroy() end)
    update()
end
...
function DeusUI:CreateWindow(config)
    ...
    local Topbar = self:Create("Frame", { Name = "Topbar", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 45), BackgroundTransparency = 1, Active = true })
    self:Create("TextLabel", { Name = "Title", Parent = Topbar, Text = config.Title or "Deus Evolution", Size = UDim2.new(1, -100, 1, 0), Position = UDim2.new(0, 15, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 16, Font = Enum.Font.GothamBold, TextXAlignment = "Left" })
    local Sep = self:Create("Frame", { Name = "Separator", Parent = Topbar, Size = UDim2.new(1, -30, 0, 1), Position = UDim2.new(0, 15, 1, 0), BackgroundColor3 = theme.Outline, BackgroundTransparency = 0.9, BorderSizePixel = 0 })
    
    local Controls = self:Create("Frame", { Name = "Controls", Parent = Topbar, Size = UDim2.new(0, 75, 1, 0), Position = UDim2.new(1, -85, 0, 0), BackgroundTransparency = 1, ZIndex = 10 })
    ...
    local MinBtn = self:Create("TextButton", { Name = "Min", Parent = Controls, Size = UDim2.new(0, 26, 0, 26), BackgroundColor3 = theme.Secondary, Text = "-", TextColor3 = theme.Text, TextSize = 18, Font = Enum.Font.GothamBold, ZIndex = 11 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), self:Create("UIStroke", {Color = theme.Outline, Transparency = 0.8}) })
    local CloseBtn = self:Create("TextButton", { Name = "Close", Parent = Controls, Size = UDim2.new(0, 26, 0, 26), BackgroundColor3 = Color3.fromRGB(180, 40, 40), Text = "×", TextColor3 = Color3.new(1, 1, 1), TextSize = 18, Font = Enum.Font.GothamBold, ZIndex = 11 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) })
    ...
    Topbar.InputBegan:Connect(function(input) if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and not Minimized then dragging = true; dragStart = input.Position; startPos = MainFrame.Position; input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then dragging = false end end) end end)
    ...
    function Window:Tab(tabConfig)
        local tabTitle = tabConfig.Title or "Tab"; local Page = DeusUI:Create("ScrollingFrame", { Name = tabTitle .. "Page", Parent = PageContainer, Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Visible = false, ScrollBarThickness = 2, ScrollBarImageColor3 = theme.Accent, AutomaticCanvasSize = "Y" }, { DeusUI:Create("UIListLayout", {Padding = UDim.new(0, 8), HorizontalAlignment = "Center"}) })
        ...
        TabBtn.MouseButton1Click:Connect(function() for _, p in pairs(PageContainer:GetChildren()) do p.Visible = false end; for _, el in pairs(DeusUI.Elements) do if el.Type == "TabBtn" then el.Selected = false; el.Instance.BackgroundColor3 = DeusUI.Themes[DeusUI.CurrentTheme].Main; el.Instance.TextColor3 = DeusUI.Themes[DeusUI.CurrentTheme].TextSecondary end end; Page.Visible = true; tabObj.Selected = true; TabBtn.BackgroundColor3 = DeusUI.Themes[DeusUI.CurrentTheme].Accent; TabBtn.TextColor3 = DeusUI.Themes[DeusUI.CurrentTheme].Text end)
        ...
        function Tab:Button(btnConfig) local ButtonFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 40), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.3 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.85, Thickness = 1.2}), DeusUI:Create("TextButton", {Name = "Btn", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = btnConfig.Title or "Button", TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamBold}) }); ButtonFrame.Btn.MouseButton1Click:Connect(function() btnConfig.Callback() end); table.insert(DeusUI.Elements, {Type = "Button", Instance = ButtonFrame}); applyLock(ButtonFrame, btnConfig.Locked, btnConfig.LockMessage) end
        function Tab:Toggle(tConfig) local toggled = tConfig.Default or false; local ToggleFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = tConfig.Title or "Toggle", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -60, 1, 0), TextXAlignment = "Left"}) }); local Switch = DeusUI:Create("TextButton", { Parent = ToggleFrame, Size = UDim2.new(0, 42, 0, 22), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = toggled and DeusUI.Themes[DeusUI.CurrentTheme].Accent or Color3.fromRGB(60, 60, 60), Text = "", AutoButtonColor = false }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), DeusUI:Create("UIStroke", {Color = Color3.new(1,1,1), Transparency = 0.8, Thickness = 1.2}) }); local Circle = DeusUI:Create("Frame", { Parent = Switch, Size = UDim2.new(0, 18, 0, 18), Position = toggled and UDim2.new(1, -2, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), AnchorPoint = toggled and Vector2.new(1, 0.5) or Vector2.new(0, 0.5), BackgroundColor3 = Color3.new(1, 1, 1) }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }); Switch.MouseButton1Click:Connect(function() toggled = not toggled; DeusUI:Tween(Circle, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {Position = toggled and UDim2.new(1, -2, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), AnchorPoint = toggled and Vector2.new(1, 0.5) or Vector2.new(0, 0.5)}); DeusUI:Tween(Switch, TweenInfo.new(0.25, Enum.EasingStyle.Quart), {BackgroundColor3 = toggled and DeusUI.Themes[DeusUI.CurrentTheme].Accent or Color3.fromRGB(60, 60, 60)}); tConfig.Callback(toggled) end); table.insert(DeusUI.Elements, {Type = "Text", Instance = ToggleFrame.T}); applyLock(ToggleFrame, tConfig.Locked, tConfig.LockMessage) end
        function Tab:Slider(sConfig) local min, max, val = sConfig.Min or 0, sConfig.Max or 100, sConfig.Default or 50; local SliderFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 65), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIPadding", {PaddingTop = UDim.new(0, 10), PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = sConfig.Title or "Slider", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(0.7, 0, 0, 20), TextXAlignment = "Left"}), DeusUI:Create("TextLabel", {Name = "Val", Text = tostring(val), BackgroundTransparency = 1, TextColor3 = theme.Accent, TextSize = 14, Font = Enum.Font.GothamBold, Size = UDim2.new(0.3, 0, 0, 20), Position = UDim2.new(0.7, 0, 0, 0), TextXAlignment = "Right"}) }); local Bar = DeusUI:Create("Frame", {Parent = SliderFrame, Size = UDim2.new(1, 0, 0, 4), Position = UDim2.new(0, 0, 0, 40), BackgroundColor3 = theme.Main, BackgroundTransparency = 0.6}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }); local Fill = DeusUI:Create("Frame", {Parent = Bar, Size = UDim2.new((val-min)/(max-min), 0, 1, 0), BackgroundColor3 = theme.Accent}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }); local Knob = DeusUI:Create("Frame", {Parent = Fill, Size = UDim2.new(0, 16, 0, 16), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1, 1, 1)}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), DeusUI:Create("UIStroke", {Color = theme.Accent, Thickness = 2}) }); local dragging = false; local function update(input) local pos = math.clamp((input.Position.X - Bar.AbsolutePosition.X) / Bar.AbsoluteSize.X, 0, 1); val = math.floor(min + (max-min)*pos); SliderFrame.Val.Text = tostring(val); Fill.Size = UDim2.new(pos, 0, 1, 0); sConfig.Callback(val) end; Bar.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = true; update(input) end end); UserInputService.InputChanged:Connect(function(input) if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then update(input) end end); UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = false end end); table.insert(DeusUI.Elements, {Type = "Text", Instance = SliderFrame.T}); table.insert(DeusUI.Elements, {Type = "AccentText", Instance = SliderFrame.Val}); applyLock(SliderFrame, sConfig.Locked, sConfig.LockMessage) end
        function Tab:Dropdown(dConfig) local dValues = dConfig.Values or {}; local selected = dConfig.Default or "Select..."; local opened = false; local DropdownFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4, ClipsDescendants = true }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}), DeusUI:Create("UIPadding", {PaddingTop = UDim.new(0, 10), PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}) }); local Header = DeusUI:Create("TextButton", {Parent = DropdownFrame, Size = UDim2.new(1, 0, 0, 25), BackgroundTransparency = 1, Text = ""}); local Title = DeusUI:Create("TextLabel", {Text = dConfig.Title or "Dropdown", Parent = Header, Size = UDim2.new(0.5, 0, 1, 0), BackgroundTransparency = 1, TextColor3 = theme.Text, Font = Enum.Font.GothamMedium, TextSize = 14, TextXAlignment = "Left"}); local SelText = DeusUI:Create("TextLabel", {Text = selected, Parent = Header, Size = UDim2.new(0.5, -25, 1, 0), Position = UDim2.new(0.5, 0, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.Accent, Font = Enum.Font.Gotham, TextSize = 13, TextXAlignment = "Right"}); local Arrow = DeusUI:Create("ImageLabel", {Parent = Header, Image = "rbxassetid://6034818372", Size = UDim2.new(0, 20, 0, 20), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundTransparency = 1, ImageColor3 = theme.TextSecondary}); local OptionHolder = DeusUI:Create("ScrollingFrame", {Parent = DropdownFrame, Size = UDim2.new(1, 0, 0, 120), Position = UDim2.new(0, 0, 0, 40), BackgroundTransparency = 1, ScrollBarThickness = 0, CanvasSize = UDim2.new(0, 0, 0, 0), AutomaticCanvasSize = "Y", Visible = false}, { DeusUI:Create("UIListLayout", {Padding = UDim.new(0, 5)}) }); Header.MouseButton1Click:Connect(function() opened = not opened; DeusUI:Tween(DropdownFrame, TweenInfo.new(0.3), {Size = opened and UDim2.new(0.95, 0, 0, 170) or UDim2.new(0.95, 0, 0, 45)}); DeusUI:Tween(Arrow, TweenInfo.new(0.3), {Rotation = opened and 180 or 0}); OptionHolder.Visible = true end); for _, v in pairs(dValues) do local Opt = DeusUI:Create("TextButton", {Parent = OptionHolder, Size = UDim2.new(1, 0, 0, 28), BackgroundColor3 = theme.Main, Text = v, TextColor3 = theme.TextSecondary, Font = Enum.Font.Gotham, TextSize = 13}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) }); Opt.MouseButton1Click:Connect(function() selected = v; SelText.Text = v; opened = false; DeusUI:Tween(DropdownFrame, TweenInfo.new(0.3), {Size = UDim2.new(0.95, 0, 0, 45)}); dConfig.Callback(v) end) end; table.insert(DeusUI.Elements, {Type = "Text", Instance = Title}); table.insert(DeusUI.Elements, {Type = "AccentText", Instance = SelText}); applyLock(DropdownFrame, dConfig.Locked, dConfig.LockMessage) end
        function Tab:Keybind(kConfig) local currentKey = kConfig.Default or Enum.KeyCode.F; local BindFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = kConfig.Title or "Keybind", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -100, 1, 0), TextXAlignment = "Left"}) }); local Btn = DeusUI:Create("TextButton", { Parent = BindFrame, Size = UDim2.new(0, 80, 0, 25), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = theme.Main, Text = currentKey.Name, TextColor3 = theme.Accent, Font = Enum.Font.GothamBold, TextSize = 12 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}) }); Btn.MouseButton1Click:Connect(function() Btn.Text = "..."; local connection; connection = UserInputService.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.Keyboard then currentKey = input.KeyCode; Btn.Text = currentKey.Name; connection:Disconnect(); kConfig.Callback(currentKey) end end) end); table.insert(DeusUI.Elements, {Type = "Text", Instance = BindFrame.T}); applyLock(BindFrame, kConfig.Locked, kConfig.LockMessage) end
        function Tab:Colorpicker(cConfig) local current = cConfig.Default or Color3.fromRGB(0, 162, 255); local ColorFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIStroke", {Color = theme.Outline, Transparency = 0.9}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}), DeusUI:Create("TextLabel", {Name = "T", Text = cConfig.Title or "Color", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -60, 1, 0), TextXAlignment = "Left"}) }); local ColorView = DeusUI:Create("TextButton", { Parent = ColorFrame, Size = UDim2.new(0, 40, 0, 22), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = current, Text = "" }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}) }); ColorView.MouseButton1Click:Connect(function() DeusUI:OpenColorPicker(current, function(nc) current = nc; ColorView.BackgroundColor3 = nc; cConfig.Callback(nc) end) end); table.insert(DeusUI.Elements, {Type = "Text", Instance = ColorFrame.T}); applyLock(ColorFrame, cConfig.Locked, cConfig.LockMessage) end
        function Tab:Paragraph(pConfig) local ParaFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(0.95, 0, 0, 0), AutomaticSize = "Y", BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.6 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), DeusUI:Create("UIPadding", {PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 12), PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12)}), DeusUI:Create("UIListLayout", {Padding = UDim.new(0, 4)}), DeusUI:Create("TextLabel", {Name = "T", Text = pConfig.Title or "Info", BackgroundTransparency = 1, TextColor3 = theme.Accent, Font = Enum.Font.GothamBold, TextSize = 14, AutomaticSize = "XY"}), DeusUI:Create("TextLabel", {Name = "D", Text = pConfig.Desc or "", BackgroundTransparency = 1, TextColor3 = theme.TextSecondary, Font = Enum.Font.Gotham, TextSize = 13, AutomaticSize = "Y", Size = UDim2.new(1, 0, 0, 0), TextWrapped = true, TextXAlignment = "Left"}) }); table.insert(DeusUI.Elements, {Type = "AccentText", Instance = ParaFrame.T}); table.insert(DeusUI.Elements, {Type = "Text", Instance = ParaFrame.D}) end
        return Tab
    end
    ...
    task.spawn(function() while MainFrame.Parent do local fps = math.floor(workspace:GetRealPhysicsFPS()); local ping = math.floor(LocalPlayer:GetNetworkPing() * 1000); StatusBar.PerfText.Text = string.format("FPS: %d | Ping: %dms | Deus Evolution v1.2.7", fps, ping); task.wait(1) end end)
    return Window
end
