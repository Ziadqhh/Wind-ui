...
local DeusUI = {
    Themes = {
        Dark = { Main = Color3.fromRGB(15, 16, 22), Secondary = Color3.fromRGB(20, 21, 28), ChannelBG = Color3.fromRGB(12, 13, 18), Outline = Color3.fromRGB(40, 42, 54), Text = Color3.fromRGB(240, 240, 245), TextSecondary = Color3.fromRGB(140, 145, 170), Accent = Color3.fromRGB(130, 115, 255), Success = Color3.fromRGB(80, 250, 123), Danger = Color3.fromRGB(255, 85, 85), Transparency = 0.05 },
        Midnight = { Main = Color3.fromRGB(8, 9, 13), Secondary = Color3.fromRGB(13, 14, 20), ChannelBG = Color3.fromRGB(5, 6, 9), Outline = Color3.fromRGB(28, 30, 42), Text = Color3.fromRGB(255, 255, 255), TextSecondary = Color3.fromRGB(110, 115, 140), Accent = Color3.fromRGB(0, 210, 255), Success = Color3.fromRGB(0, 255, 170), Danger = Color3.fromRGB(255, 60, 100), Transparency = 0.1 }
    },
    Icons = {},
...
function DeusUI:Welcome(config)
    ...
    local MainFrame = self:Create("CanvasGroup", { Name = "SplashMain", Parent = SplashGui, Size = config.Size or UDim2.new(0, 320, 0, 320), Position = config.Position or UDim2.new(0.5, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = config.Background or theme.Main, BackgroundTransparency = config.Transparency or 0.05, GroupTransparency = 1, ClipsDescendants = true }, { 
        self:Create("UICorner", {CornerRadius = UDim.new(0, 12)}), 
        self:Create("UIStroke", {Color = theme.Accent, Thickness = 1.5, Transparency = 0.4}), 
        self:Create("UIScale", {Scale = 0.9}) 
    })
    if config.Icon ~= false then 
        self:Create("ImageLabel", { Name = "Logo", Parent = MainFrame, Size = config.IconSize or UDim2.new(0, 70, 0, 70), Position = config.IconPosition or UDim2.new(0.5, 0, 0.4, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, Image = self:GetIcon(config.Icon or "shield-check"), ImageColor3 = theme.Accent, ZIndex = 2 }) 
    end
    self:Create("TextLabel", { Name = "Title", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 30), Position = config.TitlePosition or UDim2.new(0, 0, 0.62, 0), BackgroundTransparency = 1, Text = config.Title or "DEUS EVOLUTION", TextColor3 = theme.Text, TextSize = 18, Font = Enum.Font.GothamBold, ZIndex = 2 })
    self:Create("TextLabel", { Name = "Desc", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 20), Position = config.DescPosition or UDim2.new(0, 0, 0.72, 0), BackgroundTransparency = 1, Text = config.Desc or "System Loading...", TextColor3 = theme.TextSecondary, TextSize = 11, Font = Enum.Font.GothamMedium, ZIndex = 2 })
    if config.ShowProgress ~= false then 
        local BarBG = self:Create("Frame", { Name = "BarBG", Parent = MainFrame, Size = config.BarSize or UDim2.new(0.7, 0, 0, 3), Position = config.BarPosition or UDim2.new(0.5, 0, 0.88, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = theme.Secondary, ZIndex = 2 }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) })
        local BarFill = self:Create("Frame", { Name = "Fill", Parent = BarBG, Size = UDim2.new(0, 0, 1, 0), BackgroundColor3 = config.BarColor or theme.Accent, ZIndex = 3 }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) })
        self:Tween(BarFill, TweenInfo.new(config.Duration or 3, Enum.EasingStyle.Quart), {Size = UDim2.new(1, 0, 1, 0)}) 
    end
    self:Tween(MainFrame, TweenInfo.new(0.8, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {GroupTransparency = 0}); self:Tween(MainFrame.UIScale, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1})
    task.wait((config.Duration or 3) + 0.5)
    self:Tween(MainFrame, TweenInfo.new(0.8, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {GroupTransparency = 1}); self:Tween(MainFrame.UIScale, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Scale = 0.9})
    task.wait(0.8); SplashGui:Destroy()
end
...
function DeusUI:UpdateUI()
    ...
    for _, data in pairs(self.Elements) do
        ...
        elseif data.Type == "Sidebar" then self:Tween(inst, TI, {BackgroundColor3 = theme.Secondary, BackgroundTransparency = theme.Transparency + 0.2})
        elseif data.Type == "Topbar" then self:Tween(inst, TI, {BackgroundColor3 = theme.Secondary, BackgroundTransparency = theme.Transparency})
        elseif data.Type == "TabBtn" then
            ...
            if data.Selected then self:Tween(inst, TI, {BackgroundTransparency = 0.9, BackgroundColor3 = theme.Accent}); self:Tween(inst.Indicator, TI, {Size = UDim2.new(0.8, 0, 0, 2), BackgroundTransparency = 0, BackgroundColor3 = theme.Accent})
            else self:Tween(inst, TI, {BackgroundTransparency = 1}); self:Tween(inst.Indicator, TI, {Size = UDim2.new(0, 0, 0, 2), BackgroundTransparency = 1}) end
...
function DeusUI:Notify(config)
    ...
    if not self.NotifyGui then 
        ...
        self.NotifyHolder = self:Create("Frame", { Name = "Holder", Parent = self.NotifyGui, Size = UDim2.new(0, 300, 1, -20), Position = UDim2.new(1, -310, 0, 10), BackgroundTransparency = 1 }, { self:Create("UIListLayout", {VerticalAlignment = "Bottom", Padding = UDim.new(0, 12), HorizontalAlignment = "Right"}) }) 
    end
    local Toast = self:Create("Frame", { Name = "Toast", Parent = self.NotifyHolder, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = "Y", BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.1, ClipsDescendants = true }, { 
        self:Create("UICorner", {CornerRadius = UDim.new(0, 8)}), 
        self:Create("UIStroke", {Color = theme.Outline, Transparency = 0.5, Thickness = 1}),
        self:Create("UIPadding", {PaddingTop = UDim.new(0, 12), PaddingBottom = UDim.new(0, 12), PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}),
        self:Create("UIListLayout", {Padding = UDim.new(0, 4)}),
        self:Create("TextLabel", { Name = "Title", Text = config.Title or "SYSTEM", BackgroundTransparency = 1, TextColor3 = theme.Accent, TextSize = 13, Font = Enum.Font.GothamBold, TextXAlignment = "Left", AutomaticSize = "XY" }), 
        self:Create("TextLabel", { Name = "Content", Text = config.Content or "Message", BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 11, Font = Enum.Font.GothamMedium, TextWrapped = true, TextXAlignment = "Left", Size = UDim2.new(1, 0, 0, 0), AutomaticSize = "Y" }), 
        self:Create("Frame", { Name = "Progress", Size = UDim2.new(1, 0, 0, 2), Position = UDim2.new(0, 0, 1, 0), BackgroundColor3 = theme.Accent, BackgroundTransparency = 0.4, ZIndex = 2 }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }) 
    })
    Toast.Position = UDim2.new(1.5, 0, 0, 0); 
    self:Tween(Toast, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0, 0, 0, 0)}); 
    local dur = config.Duration or 4; 
    self:Tween(Toast.Progress, TweenInfo.new(dur, Enum.EasingStyle.Linear), {Size = UDim2.new(0, 0, 0, 2)}); 
    task.delay(dur, function() if Toast then self:Tween(Toast, TweenInfo.new(0.6, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Position = UDim2.new(1.5, 0, 0, 0), BackgroundTransparency = 1}); task.wait(0.6); Toast:Destroy() end end)
end
...
function DeusUI:CreateWindow(config)
    ...
    local WindowSize = UDim2.new(0, 520, 0, 380); local MainFrame = self:Create("CanvasGroup", { Name = "MainFrame", Parent = ScreenGui, BackgroundColor3 = theme.Main, Position = UDim2.new(0.5, -260, 0.5, -190), Size = WindowSize, ClipsDescendants = true, ZIndex = 1, GroupTransparency = 0 }, { self:Create("UICorner", {CornerRadius = UDim.new(0, 10)}), self:Create("UIStroke", {Color = theme.Outline, Transparency = 0.3, Thickness = 1.2, ApplyStrokeMode = Enum.ApplyStrokeMode.Border}), self:Create("UIScale", {Scale = 1}) }); self.MainFrame = MainFrame; self.UIScale = MainFrame.UIScale; table.insert(self.Elements, {Instance = MainFrame, Type = "MainFrame"})
    local Topbar = self:Create("Frame", { Name = "Topbar", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 45), Position = UDim2.new(0, 0, 0, 0), BackgroundColor3 = theme.Secondary, ZIndex = 3 }, { self:Create("Frame", {Name = "TopLine", Size = UDim2.new(1, 0, 0, 2), Position = UDim2.new(0, 0, 0, 0), BackgroundColor3 = theme.Accent, ZIndex = 4}), self:Create("TextLabel", {Name = "Title", Text = config.Title or "DEUS EVOLUTION", Size = UDim2.new(0, 200, 1, 0), Position = UDim2.new(0, 20, 0, 0), BackgroundTransparency = 1, TextColor3 = theme.Text, TextSize = 14, Font = Enum.Font.GothamBold, TextXAlignment = "Left", ZIndex = 4}) }); table.insert(self.Elements, {Instance = Topbar, Type = "Topbar"})
    local Sidebar = self:Create("Frame", { Name = "Navbar", Parent = MainFrame, Size = UDim2.new(1, 0, 0, 40), Position = UDim2.new(0, 0, 0, 45), BackgroundColor3 = theme.Secondary, BackgroundTransparency = 0.3, ZIndex = 2 }, { self:Create("Frame", {Size = UDim2.new(1, -40, 0, 1), Position = UDim2.new(0, 20, 1, 0), BackgroundColor3 = theme.Outline, BackgroundTransparency = 0.7}) }); table.insert(self.Elements, {Instance = Sidebar, Type = "Sidebar"})
    local TabContainer = self:Create("ScrollingFrame", { Name = "TabScroll", Parent = Sidebar, Size = UDim2.new(1, -20, 1, 0), Position = UDim2.new(0, 10, 0, 0), BackgroundTransparency = 1, ScrollBarThickness = 0, AutomaticCanvasSize = Enum.AutomaticSize.X, CanvasSize = UDim2.new(0,0,0,0), ZIndex = 3 }, { self:Create("UIListLayout", {FillDirection = "Horizontal", Padding = UDim.new(0, 12), VerticalAlignment = "Center", SortOrder = Enum.SortOrder.LayoutOrder}), self:Create("UIPadding", {PaddingLeft = UDim.new(0, 15), PaddingRight = UDim.new(0, 15)}) }); local PageContainer = self:Create("Frame", { Name = "PageContainer", Parent = MainFrame, Size = UDim2.new(1, -30, 1, -100), Position = UDim2.new(0, 15, 0, 95), BackgroundTransparency = 1, ZIndex = 2 }); self.Topbar = Topbar
    ...
    function WindowMethods:SidebarSection(title) OrderTracker = OrderTracker + 1; local Sec = DeusUI:Create("TextLabel", { Name = "Section_" .. title, Parent = TabContainer, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = "X", BackgroundTransparency = 1, Text = title:upper(), TextColor3 = DeusUI.Themes[DeusUI.CurrentTheme].TextSecondary, TextSize = 10, Font = Enum.Font.GothamBold, ZIndex = 4, LayoutOrder = OrderTracker }, { DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 5), PaddingRight = UDim.new(0, 5)}) }); return Sec end
    function WindowMethods:Tab(tabConfig)
        OrderTracker = OrderTracker + 1; local Page = DeusUI:Create("ScrollingFrame", { Parent = PageContainer, Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Visible = false, ScrollBarThickness = 1, ScrollBarImageColor3 = theme.Outline, AutomaticCanvasSize = Enum.AutomaticSize.Y, CanvasSize = UDim2.new(0,0,0,0), ZIndex = 3 }, { DeusUI:Create("UIListLayout", {Padding = UDim.new(0, 10), HorizontalAlignment = "Center"}), DeusUI:Create("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5)}) }); local tabTitle = tabConfig.Title or "Tab"; local tabIcon = DeusUI:GetIcon(tabConfig.Icon)
        local TabBtn = DeusUI:Create("TextButton", { Parent = TabContainer, Size = UDim2.new(0, 0, 0, 30), AutomaticSize = "X", BackgroundTransparency = 1, Text = "", AutoButtonColor = false, ZIndex = 4, LayoutOrder = OrderTracker }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), DeusUI:Create("Frame", { Name = "Indicator", Size = UDim2.new(0, 0, 0, 2), Position = UDim2.new(0.5, 0, 1, 4), AnchorPoint = Vector2.new(0.5, 1), BackgroundColor3 = theme.Accent, BackgroundTransparency = 1, ZIndex = 5 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }), DeusUI:Create("Frame", { Name = "Content", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, ZIndex = 5 }, { DeusUI:Create("UIListLayout", { FillDirection = "Horizontal", VerticalAlignment = "Center", Padding = UDim.new(0, 8), HorizontalAlignment = "Center" }), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12)}), tabIcon and DeusUI:Create("ImageLabel", { Name = "Icon", Size = UDim2.new(0, 16, 0, 16), BackgroundTransparency = 1, Image = tabIcon, ImageColor3 = theme.TextSecondary, ZIndex = 6 }) or nil, DeusUI:Create("TextLabel", { Name = "Label", Text = tabTitle, Size = UDim2.new(0, 0, 1, 0), AutomaticSize = "X", BackgroundTransparency = 1, TextColor3 = theme.TextSecondary, TextSize = 12, Font = Enum.Font.GothamMedium, ZIndex = 6 }) }) })
        local tabData = {Type = "TabBtn", Instance = TabBtn, Selected = false}; table.insert(DeusUI.Elements, tabData)
        local function SetTabState(state) tabData.Selected = state; local cur = DeusUI.Themes[DeusUI.CurrentTheme]; local targetText = state and cur.Text or cur.TextSecondary; DeusUI:Tween(TabBtn.Content.Label, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {TextColor3 = targetText}); if TabBtn.Content:FindFirstChild("Icon") then DeusUI:Tween(TabBtn.Content.Icon, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {ImageColor3 = targetText}) end; if state then DeusUI:Tween(TabBtn, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {BackgroundTransparency = 0.92, BackgroundColor3 = cur.Accent}); DeusUI:Tween(TabBtn.Indicator, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0.7, 0, 0, 2), BackgroundTransparency = 0}) else DeusUI:Tween(TabBtn, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {BackgroundTransparency = 1}); DeusUI:Tween(TabBtn.Indicator, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(0, 0, 0, 2), BackgroundTransparency = 1}) end end
        ...
        TabBtn.MouseButton1Click:Connect(function() for _, p in pairs(PageContainer:GetChildren()) do if p:IsA("ScrollingFrame") then p.Visible = false end end; for _, el in pairs(DeusUI.Elements) do if el.Type == "TabBtn" then el.Selected = false; local eInst = el.Instance; DeusUI:Tween(eInst, TweenInfo.new(0.2), {BackgroundTransparency = 1}); DeusUI:Tween(eInst.Indicator, TweenInfo.new(0.2), {Size = UDim2.new(0, 0, 0, 2), BackgroundTransparency = 1}); DeusUI:Tween(eInst.Content.Label, TweenInfo.new(0.2), {TextColor3 = DeusUI.Themes[DeusUI.CurrentTheme].TextSecondary}); if eInst.Content:FindFirstChild("Icon") then DeusUI:Tween(eInst.Content.Icon, TweenInfo.new(0.2), {ImageColor3 = DeusUI.Themes[DeusUI.CurrentTheme].TextSecondary}) end end end; Page.Visible = true; SetTabState(true); if Topbar:FindFirstChild("Title") then Topbar.Title.Text = (config.Title or "DEUS EVOLUTION") .. " | " .. tabTitle end end)
        if not DeusUI.SelectedTab then DeusUI.SelectedTab = true; Page.Visible = true; SetTabState(true); if Topbar:FindFirstChild("Title") then Topbar.Title.Text = (config.Title or "DEUS EVOLUTION") .. " | " .. tabTitle end end
        local TabMethods = {}
        ...
        function TabMethods:Button(btnConfig) 
            ...
            local Button = DeusUI:Create("TextButton", { Parent = Page, Size = UDim2.new(1, 0, 0, 32), BackgroundColor3 = current.Accent, Text = btnConfig.Title or "Button", TextColor3 = Color3.new(1,1,1), TextSize = 12, Font = Enum.Font.GothamBold, AutoButtonColor = false, ZIndex = 4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), DeusUI:Create("UIStroke", {Color = Color3.new(1,1,1), Transparency = 0.8, Thickness = 1}) }); 
            table.insert(DeusUI.Elements, {Instance = Button, Type = "Button"}); 
            Button.MouseEnter:Connect(function() DeusUI:Tween(Button, TweenInfo.new(0.2), {BackgroundColor3 = current.Accent:Lerp(Color3.new(1,1,1), 0.1)}) end)
            Button.MouseLeave:Connect(function() DeusUI:Tween(Button, TweenInfo.new(0.2), {BackgroundColor3 = current.Accent}) end)
            Button.MouseButton1Down:Connect(function() DeusUI:Tween(Button, TweenInfo.new(0.1), {Size = UDim2.new(1, -4, 0, 30)}) end)
            Button.MouseButton1Up:Connect(function() DeusUI:Tween(Button, TweenInfo.new(0.1), {Size = UDim2.new(1, 0, 0, 32)}) end)
            Button.MouseButton1Click:Connect(function() btnConfig.Callback() end) 
        end
        function TabMethods:Toggle(tConfig)
            local toggled = tConfig.Default or false; local current = DeusUI.Themes[DeusUI.CurrentTheme]; 
            local ToggleFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(1, 0, 0, 35), BackgroundColor3 = current.Secondary, BackgroundTransparency = 0.5, ZIndex = 4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12)}), DeusUI:Create("TextLabel", {Name = "T", Text = tConfig.Title or "Toggle", BackgroundTransparency = 1, TextColor3 = current.Text, TextSize = 12, Font = Enum.Font.GothamMedium, Size = UDim2.new(1, -50, 1, 0), TextXAlignment = "Left", ZIndex = 5}) }); 
            table.insert(DeusUI.Elements, {Instance = ToggleFrame.T, Type = "TextColor"}); 
            local Switch = DeusUI:Create("TextButton", { Parent = ToggleFrame, Size = UDim2.new(0, 36, 0, 18), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(1, 0.5), BackgroundColor3 = toggled and current.Success or current.ChannelBG, Text = "", ZIndex = 5 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), DeusUI:Create("UIStroke", {Color = current.Outline, Transparency = 0.5, Thickness = 1.2}) }); 
            ...
            Switch.MouseButton1Click:Connect(function() 
                ...
                DeusUI:Tween(Circle, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Position = toggled and UDim2.new(1, -2, 0.5, 0) or UDim2.new(0, 2, 0.5, 0), AnchorPoint = toggled and Vector2.new(1, 0.5) or Vector2.new(0, 0.5)}); 
                DeusUI:Tween(Switch, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {BackgroundColor3 = toggled and current.Success or current.ChannelBG}); 
                tConfig.Callback(toggled) 
            end)
        end
        function TabMethods:Slider(sConfig)
            local min, max, val = sConfig.Min or 0, sConfig.Max or 100, sConfig.Default or 50; local current = DeusUI.Themes[DeusUI.CurrentTheme]; 
            local SliderFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(1, 0, 0, 45), BackgroundColor3 = current.Secondary, BackgroundTransparency = 0.5, ZIndex = 4 }, { 
                DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), DeusUI:Create("UIPadding", {PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12)}),
                DeusUI:Create("TextLabel", {Name = "T", Text = sConfig.Title or "Slider", BackgroundTransparency = 1, TextColor3 = current.Text, TextSize = 12, Font = Enum.Font.GothamMedium, Size = UDim2.new(0.7, 0, 0, 20), Position = UDim2.new(0, 0, 0, 5), TextXAlignment = "Left", ZIndex = 5}), 
                DeusUI:Create("TextLabel", {Name = "Val", Text = tostring(val), BackgroundTransparency = 1, TextColor3 = current.Accent, TextSize = 11, Font = Enum.Font.GothamBold, Size = UDim2.new(0.3, 0, 0, 20), Position = UDim2.new(0.7, 0, 0, 5), TextXAlignment = "Right", ZIndex = 5}) 
            }); 
            table.insert(DeusUI.Elements, {Instance = SliderFrame.T, Type = "TextColor"}); 
            local Bar = DeusUI:Create("Frame", {Parent = SliderFrame, Size = UDim2.new(1, 0, 0, 4), Position = UDim2.new(0, 0, 0, 32), BackgroundColor3 = current.ChannelBG, ZIndex = 5}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }); 
            table.insert(DeusUI.Elements, {Instance = Bar, Type = "SecondaryBG"}); 
            local Fill = DeusUI:Create("Frame", {Parent = Bar, Size = UDim2.new((val-min)/(max-min), 0, 1, 0), BackgroundColor3 = current.Accent, ZIndex = 6}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }); 
            table.insert(DeusUI.Elements, {Instance = Fill, Type = "SliderFill"}); 
            local Knob = DeusUI:Create("Frame", {Parent = Fill, Size = UDim2.new(0, 12, 0, 12), Position = UDim2.new(1, 0, 0.5, 0), AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.new(1, 1, 1), ZIndex = 7}, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(1, 0)}), DeusUI:Create("UIStroke", {Color = current.Accent, Thickness = 2}) }); 
            local dragging = false; local function update(input) local pos = math.clamp((input.Position.X - Bar.AbsolutePosition.X) / Bar.AbsoluteSize.X, 0, 1); val = math.floor(min + (max - min) * pos); SliderFrame.Val.Text = tostring(val); Fill.Size = UDim2.new(pos, 0, 1, 0); sConfig.Callback(val) end; 
        ...
        function TabMethods:Dropdown(dConfig)
            ...
            local DropdownFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(1, 0, 0, 36), BackgroundColor3 = current.Secondary, BackgroundTransparency = 0.5, ClipsDescendants = true, ZIndex = 4 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), DeusUI:Create("UIStroke", {Color = current.Outline, Transparency = 0.6, Thickness = 1}) }); 
            ...
            local Title = DeusUI:Create("TextLabel", {Text = dConfig.Title or "Dropdown", Parent = Header, Size = UDim2.new(1, -120, 1, 0), Position = UDim2.new(0, 15, 0, 0), BackgroundTransparency = 1, TextColor3 = current.Text, Font = Enum.Font.GothamMedium, TextSize = 12, TextXAlignment = "Left", ZIndex = 6}); 
            table.insert(DeusUI.Elements, {Instance = Title, Type = "TextColor"}); 
            local SelText = DeusUI:Create("TextLabel", {Text = selected, Parent = Header, Size = UDim2.new(0, 100, 1, 0), Position = UDim2.new(1, -30, 0, 0), AnchorPoint = Vector2.new(1, 0), BackgroundTransparency = 1, TextColor3 = current.Accent, Font = Enum.Font.GothamMedium, TextSize = 11, TextXAlignment = "Right", ZIndex = 6}); 
            local Arrow = DeusUI:Create("TextLabel", {Text = "▼", Parent = Header, Size = UDim2.new(0, 20, 1, 0), Position = UDim2.new(1, -5, 0, 0), AnchorPoint = Vector2.new(1, 0), BackgroundTransparency = 1, TextColor3 = current.TextSecondary, Font = Enum.Font.Gotham, TextSize = 10, ZIndex = 6}); 
            local OptionHolder = DeusUI:Create("ScrollingFrame", {Parent = DropdownFrame, Size = UDim2.new(1, 0, 0, 120), Position = UDim2.new(0, 0, 0, 36), BackgroundTransparency = 1, ScrollBarThickness = 1, AutomaticCanvasSize = "Y", Visible = false, ZIndex = 10}, { DeusUI:Create("UIListLayout", {Padding = UDim.new(0, 4), HorizontalAlignment = "Center"}), DeusUI:Create("UIPadding", {PaddingTop = UDim.new(0, 5), PaddingBottom = UDim.new(0, 5)}) }); 
            local function Refresh() for _, child in pairs(OptionHolder:GetChildren()) do if child:IsA("TextButton") then child:Destroy() end end; 
                local list = (type(dValues) == "function") and dValues() or dValues; 
                for _, v in pairs(list) do 
                    local isPlayer = typeof(v) == "Instance" and v:IsA("Player"); local displayName = isPlayer and v.DisplayName or tostring(v); 
                    local btn = DeusUI:Create("TextButton", { Parent = OptionHolder, Size = UDim2.new(1, -10, 0, 30), BackgroundColor3 = current.Main, BackgroundTransparency = 0.8, Text = "", ZIndex = 11 }, { DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 4)}), DeusUI:Create("TextLabel", { Text = displayName, Size = UDim2.new(1, isPlayer and -40 or -20, 1, 0), Position = UDim2.new(0, isPlayer and 35 or 10, 0, 0), BackgroundTransparency = 1, TextColor3 = current.TextSecondary, Font = Enum.Font.Gotham, TextSize = 11, TextXAlignment = "Left", ZIndex = 12 }) }); 
                    if isPlayer then self:Create("ImageLabel", { Parent = btn, Size = UDim2.new(0, 22, 0, 22), Position = UDim2.new(0, 6, 0.5, 0), AnchorPoint = Vector2.new(0, 0.5), BackgroundTransparency = 1, Image = Players:GetUserThumbnailAsync(v.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48), ZIndex = 12 }, { self:Create("UICorner", {CornerRadius = UDim.new(1, 0)}) }) end; 
                    btn.MouseEnter:Connect(function() DeusUI:Tween(btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.5, BackgroundColor3 = current.Accent}) end)
                    btn.MouseLeave:Connect(function() DeusUI:Tween(btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.8, BackgroundColor3 = current.Main}) end)
                    btn.MouseButton1Click:Connect(function() selected = displayName; SelText.Text = displayName; opened = false; DeusUI:Tween(DropdownFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Size = UDim2.new(1, 0, 0, 36)}); OptionHolder.Visible = false; dConfig.Callback(v) end) 
                end 
            end; 
            Header.MouseButton1Click:Connect(function() opened = not opened; if opened then Refresh() end; DeusUI:Tween(DropdownFrame, TweenInfo.new(0.4, Enum.EasingStyle.Quart), {Size = opened and UDim2.new(1, 0, 0, 160) or UDim2.new(1, 0, 0, 36)}); OptionHolder.Visible = opened end)
        end
        ...
        function TabMethods:Paragraph(pConfig) 
            local current = DeusUI.Themes[DeusUI.CurrentTheme]; 
            local ParaFrame = DeusUI:Create("Frame", { Parent = Page, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = "Y", BackgroundColor3 = current.Secondary, BackgroundTransparency = 0.6, ZIndex = 4 }, { 
                DeusUI:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), DeusUI:Create("UIStroke", {Color = current.Outline, Transparency = 0.8, Thickness = 1}),
                ...
                DeusUI:Create("TextLabel", {Name = "T", Text = pConfig.Title or "Information", BackgroundTransparency = 1, TextColor3 = current.Accent, Font = Enum.Font.GothamBold, TextSize = 13, AutomaticSize = "XY", ZIndex = 5}), 
                DeusUI:Create("TextLabel", {Name = "D", Text = pConfig.Desc or "", BackgroundTransparency = 1, TextColor3 = current.TextSecondary, Font = Enum.Font.Gotham, TextSize = 11, AutomaticSize = "Y", Size = UDim2.new(1, 0, 0, 0), TextWrapped = true, TextXAlignment = "Left", ZIndex = 5}) 
            }); 
