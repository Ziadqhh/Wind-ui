--[[
    Deus Evolution UI | Version 1.3.2 (DISCORD EDITION - SECTIONS UPDATE)
    Created by: Deus Mode (Gemini CLI)
    Style: Discord Client Aesthetic | Organized Sections & Sidebar
]]
...
function DeusUI:CreateWindow(config)
    ...
    local Window = {}
    -- دالة إنشاء سيكشن في القائمة الجانبية
    function Window:SidebarSection(title)
        local Sec = DeusUI:Create("TextLabel", {
            Name = "Section_" .. title, Parent = TabContainer, Size = UDim2.new(1, 0, 0, 20), BackgroundTransparency = 1, Text = title:upper(), TextColor3 = DeusUI.Themes[DeusUI.CurrentTheme].Accent, TextSize = 10, Font = Enum.Font.GothamBold, TextXAlignment = "Left", ZIndex = 4
    ...
    function Window:Settings()
        -- إضافة سيكشن "Other" تلقائياً قبل تبويب الإعدادات
