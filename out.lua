...
Main = (function()
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Script Dumper", IconMap = Main.LargeIcons, Icon = "Script_Viewer", OnClick = function()
			...
			print("🚀 [Script Dumper Pro] Started! Output: " .. baseDir)
			...
			task.spawn(function()
				...
				pcall(function() if not env.isfolder(baseDir) then env.makefolder(baseDir) end end)
				...
				for _, entry in ipairs(services) do
					...
					pcall(function() if not env.isfolder(servicePath) then env.makefolder(servicePath) end end)
					...
					for i, v in ipairs(allDescendants) do
						...
						if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") or v:IsA("BindableEvent") or v:IsA("BindableFunction") then
							table.insert(remoteList, v:GetFullName() .. " [" .. v.ClassName .. "]")
						end
						...
						if v:IsA("LuaSourceContainer") then
							...
							for _, part in ipairs(pathParts) do
								...
								pcall(function() if not env.isfolder(currentFolder) then env.makefolder(currentFolder) end end)
							end
							...
							if s and res then
								...
								-- Map Remotes mentioned in script
								for _, remote in ipairs(remoteList) do
									local rName = remote:match("([^%.]+) %[")
									if rName and source:find(rName) then
										logicMap[scriptFullName] = logicMap[scriptFullName] or {}
										table.insert(logicMap[scriptFullName], remote)
									end
								end
							else
								-- Failed Decompile: Try Bytecode
				...
				if env.getnilinstances then
					...
					pcall(function() if not env.isfolder(nilPath) then env.makefolder(nilPath) end end)
				...
				print("🎉 [Script Dumper Pro] Complete!")
				print("📂 Check _AI_READ_THIS.txt for a quick logic overview.")
			end)
