...
local function main()
	...
	-- StarterGui.ScreenGui.Console.Copy
	G2L["14c"] = Instance.new("ImageButton", ConsoleFrame);
	G2L["14c"]["BorderSizePixel"] = 0;
	G2L["14c"]["BackgroundColor3"] = Color3.fromRGB(57, 57, 57);
	G2L["14c"]["Size"] = UDim2.new(0, 37, 0, 15);
	G2L["14c"]["BorderColor3"] = Color3.fromRGB(0, 0, 0);
	G2L["14c"]["Name"] = [[Copy]];
	G2L["14c"]["Position"] = UDim2.new(1, -84, 0, 4);
	...
	-- StarterGui.ScreenGui.Console.Copy.TextLabel
	G2L["15c"] = Instance.new("TextLabel", G2L["14c"]);
	G2L["15c"]["TextWrapped"] = true;
	G2L["15c"]["Interactable"] = false;
	...
	-- StarterGui.ScreenGui.Console.Copy.UIPadding
	G2L["16c"] = Instance.new("UIPadding", G2L["14c"]);
	G2L["16c"]["PaddingTop"] = UDim.new(0, 1);
	G2L["16c"]["PaddingBottom"] = UDim.new(0, 1);
	...
	Console.Init = function()
		...
		Console.Copy.MouseButton1Click:Connect(function()
			local logs = {}
			for _, log in ipairs(displayedOutput) do
				if log and typeof(log) == "Instance" then
					table.insert(logs, log:GetAttribute("UnformattedText") or log.Text)
				end
			end
		...
		LogService.MessageOut:Connect(function(msg, msgtype)
			...
			newOutputText:SetAttribute("UnformattedText", unformattedText)
...
local function main()
	...
	SaveInstance.Init = function()
		...
		local BypassSec = AddCheckbox("Bypass Security (Deep Scan)", SaveInstanceArgs.BypassSecurity)
		BypassSec.OnInput:Connect(function()
			SaveInstanceArgs.BypassSecurity = BypassSec.Toggled
		end)
		
		local MapRemotes = AddCheckbox("Map Global Remotes", SaveInstanceArgs.MapRemotes)
		MapRemotes.OnInput:Connect(function()
			SaveInstanceArgs.MapRemotes = MapRemotes.Toggled
		end)
...
local function main()
	...
	ScriptViewer.Init = function()
		window = Lib.Window.new()
		window:SetTitle("📜 Script Editor")
		window:Resize(550, 400)
		ScriptViewer.Window = window

		local content = window.GuiElems.Content
		
		-- Combined Top Bar (Search + Buttons)
		local topBar = Instance.new("Frame", content)
		topBar.Size = UDim2.new(1, -10, 0, 70) -- Two rows for better mobile access
		...
		local btnRow = Instance.new("Frame", topBar)
		btnRow.Size = UDim2.new(1, 0, 0, 35)
		btnRow.Position = UDim2.new(0, 0, 0, 35)
		btnRow.BackgroundTransparency = 1

		local function createActionBtn(name, color, pos, size)
		...
		local execBtn = createActionBtn("EXEC", Color3.fromRGB(39, 174, 96), UDim2.new(0, 0, 0, 0), UDim2.new(0.23, 0, 1, 0))
		local copyBtn = createActionBtn("COPY", Color3.fromRGB(41, 128, 185), UDim2.new(0.25, 0, 0, 0), UDim2.new(0.23, 0, 1, 0))
		local saveBtn = createActionBtn("SAVE", Color3.fromRGB(142, 68, 173), UDim2.new(0.5, 0, 0, 0), UDim2.new(0.23, 0, 1, 0))
		local clearBtn = createActionBtn("CLR", Color3.fromRGB(192, 57, 43), UDim2.new(0.75, 0, 0, 0), UDim2.new(0.25, 0, 1, 0))

		-- Editor Area (Adjusted to start below topBar)
		codeFrame = Lib.CodeFrame.new()
		codeFrame.Frame.Position = UDim2.new(0, 5, 0, 80)
		codeFrame.Frame.Size = UDim2.new(1, -10, 1, -85)
		codeFrame.Frame.Parent = content

		-- Logic
		execBtn.MouseButton1Click:Connect(function() loadstring(codeFrame:GetText())() end)
		copyBtn.MouseButton1Click:Connect(function() if env.setclipboard then env.setclipboard(codeFrame:GetText()) end end)
		saveBtn.MouseButton1Click:Connect(function() Lib.SaveAsPrompt("Dump_"..os.time()..".lua", codeFrame:GetText()) end)
		clearBtn.MouseButton1Click:Connect(function() codeFrame:SetText("") end)
	end
...
Main = (function()
	...
	Main.CreateMainGui = function()
		...
		Main.CreateApp({Name = "Script Dumper", IconMap = Main.LargeIcons, Icon = "Script_Viewer", OnClick = function()
			local gameName = "Unknown"
			pcall(function() gameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name end)
			local baseDir = "DEX_DUMP_" .. gameName:gsub('[\\/:"*?<>|]', "_") .. "_" .. game.PlaceId
			
			local function sanitize(str)
				local s = str:gsub('[\\/:"*?<>|]', "_"):gsub("%c", "")
				return (s ~= "" and s) or "Unnamed"
			end

			--// FEATURE 7: JSON STRUCTURE GENERATOR
			local mapStructure = { Game = gameName, PlaceId = game.PlaceId, Hierarchy = {} }
			local function getInstanceData(inst)
				local data = {
					Name = inst.Name,
				...
				-- Only store basic properties for critical instances to save JSON size
			...
			--// FEATURE 6: EXPERT CONSTANT EXTRACTOR
			local function extractConstants(source, scriptName)
				local constants = { Strings = {}, URLs = {}, Keywords = {} }
				-- Extract URLs
				for url in source:gmatch('https?://[%w-_%.%?%&%=%/]+') do
					table.insert(constants.URLs, url)
				end
				-- Extract meaningful strings (min 5 chars, no whitespace only)
				for str in source:gmatch('["\'](.[^"\']+)["\']') do
					if #str >= 5 and not str:match("^%s*$") then
						table.insert(constants.Strings, str)
					end
				end
				return constants
			end
			...
			print("🚀 [Script Dumper Pro V2] Started! Output: " .. baseDir)
			
			task.spawn(function()
				local successCount, failCount, totalFound, bytecodeCount = 0, 0, 0, 0
				local remoteList = {}
				local remoteNames = {}
				local logicMap = {}
				local secretFound = {}
				local extractedData = {} -- For Feature 6
				local keywords = {"Webhook", "HttpService", "loadstring", "getfenv", "setfenv", "MarketplaceService", "TeleportService", "Admin", "Ban", "Kick", "Reward", "Premium"}
				
				local folderCache = {}
				local function safeMakeFolder(path)
					if folderCache[path] then return end
					local parts = path:split("/")
					local current = ""
					for i, part in ipairs(parts) do
						if part ~= "" then
							current = (current == "" and part) or (current .. "/" .. part)
							if not folderCache[current] then
								if not env.isfolder(current) then
									pcall(env.makefolder, current)
								end
								folderCache[current] = true
							end
						end
					end
				end

				safeMakeFolder(baseDir)

				print("🔍 Pre-scanning for Remotes & Building JSON...")
				for _, entry in ipairs(services) do
					local sObj = entry.service
					local serviceNode = getInstanceData(sObj)
					mapStructure.Hierarchy[entry.name] = serviceNode

					local des = sObj:GetDescendants()
					for i, v in ipairs(des) do
						...
						if v:IsA("RemoteEvent") or v:IsA("RemoteFunction") or v:IsA("BindableEvent") or v:IsA("BindableFunction") then
							...
							local entryStr = fullName .. " [" .. v.ClassName .. "]"
							table.insert(remoteList, entryStr)
							remoteNames[v.Name] = entryStr
						end
					end
					task.wait()
				end
				...
				for _, entry in ipairs(services) do
					local sName, sObj = entry.name, entry.service
					print("🔍 Scanning " .. sName .. "...")
					
					local servicePath = baseDir .. "/" .. sName
					safeMakeFolder(servicePath)

					local allDescendants = sObj:GetDescendants()
					for i, v in ipairs(allDescendants) do
						...
						if v:IsA("LuaSourceContainer") then
							...
							while parent and parent ~= sObj do
								table.insert(pathParts, 1, sanitize(parent.Name))
								parent = parent.Parent
							end
							
							local currentFolder = servicePath
							for _, part in ipairs(pathParts) do
								currentFolder = currentFolder .. "/" .. part
							end
							safeMakeFolder(currentFolder)
							
							local source = nil
							local s, res = pcall(env.decompile, v)
							if s and res then
								source = res
								successCount = successCount + 1
								
								local scriptFullName = v:GetFullName()
								--// FEATURE 6: Extract Constants automatically
								local consts = extractConstants(source, scriptFullName)
								if #consts.Strings > 0 or #consts.URLs > 0 then
									extractedData[scriptFullName] = consts
								end

								-- Optimized secret scanning
								for _, kw in ipairs(keywords) do
									if source:find(kw) then
										table.insert(secretFound, "[" .. kw .. "] in " .. scriptFullName)
									end
								end
								
								-- Optimized logic mapping
								for rName, rFull in pairs(remoteNames) do
									if source:find(rName) then
										logicMap[scriptFullName] = logicMap[scriptFullName] or {}
										table.insert(logicMap[scriptFullName], rFull)
									end
								end
							else
								failCount = failCount + 1
								if env.getscriptbytecode then
									local bs, bres = pcall(env.getscriptbytecode, v)
									if bs and bres then
										source = "-- DEX: Decompile failed, dumping bytecode instead.\n" .. bres
										bytecodeCount = bytecodeCount + 1
										local bcFileName = sanitize(v.Name) .. ".bin"
										pcall(function() env.writefile(currentFolder .. "/" .. bcFileName, bres) end)
										
										--// FEATURE 6: Try to extract strings even from bytecode
										local rawStrings = {}
										for str in bres:gmatch("[%w%s%p]{5,}") do
											table.insert(rawStrings, str)
										end
										if #rawStrings > 0 then
											extractedData[v:GetFullName() .. " (Bytecode)"] = { Strings = rawStrings, URLs = {}, Keywords = {} }
										end
									end
								end
								if not source then
									source = "-- DEX Dumper failed to decompile. Error: " .. tostring(res)
								end
							end
							
							local fileName = sanitize(v.Name) .. ".lua"
							local fullFilePath = currentFolder .. "/" .. fileName
							
							if env.isfile(fullFilePath) then
								fullFilePath = currentFolder .. "/" .. sanitize(v.Name) .. "_" .. tostring(totalFound) .. ".lua"
							end
							
							pcall(function() env.writefile(fullFilePath, source) end)
							
							if totalFound % 50 == 0 then
								print("⏳ Processed: " .. totalFound .. " scripts...")
							end
							
							task.wait()
						end
					end
					print("✅ Service " .. sName .. " complete.")
					task.wait()
				end
				...
				if env.getnilinstances then
					...
					local nilPath = baseDir .. "/_NIL"
					safeMakeFolder(nilPath)
					local nils = env.getnilinstances()
					for i, v in ipairs(nils) do
						if i % 500 == 0 then task.wait() end
						if v:IsA("LuaSourceContainer") then
							local s, res = pcall(env.decompile, v)
							local source = (s and res) or "-- Failed to decompile."
							
							--// Extract from NIL scripts too
							local consts = extractConstants(source, v.Name)
							extractedData["NIL/" .. v.Name .. "_" .. i] = consts

							local fileName = sanitize(v.Name) .. "_" .. tostring(i) .. ".lua"
							pcall(function() env.writefile(nilPath .. "/" .. fileName, source) end)
							task.wait()
						end
					end
					print("✅ NIL complete.")
				...
				pcall(function() 
					local summary = string.format("Game: %s\nPlaceId: %d\nScripts Found: %d\nDecompiled: %d\nBytecode Dumps: %d\nFailed Completely: %d\nDate: %s", gameName, game.PlaceId, totalFound, successCount, bytecodeCount, failCount - bytecodeCount, os.date())
					env.writefile(baseDir .. "/_Summary.txt", summary) 
					
					local aiSummary = "AI ANALYSIS SUMMARY\n" .. string.rep("=", 20) .. "\n"
					aiSummary = aiSummary .. "This map has " .. #remoteList .. " remotes and " .. totalFound .. " scripts.\n"
					aiSummary = aiSummary .. "Important Scripts (interacting with Remotes):\n"
					for script, rems in pairs(logicMap) do
						aiSummary = aiSummary .. "- " .. script .. " calls: " .. table.concat(rems, ", ") .. "\n"
					end
					env.writefile(baseDir .. "/_AI_READ_THIS.txt", aiSummary)

					--// SAVE FEATURE 7: JSON MAP STRUCTURE
					local HttpService = game:GetService("HttpService")
					local json = HttpService:JSONEncode(mapStructure)
					env.writefile(baseDir .. "/_Map_Structure.json", json)

					--// SAVE FEATURE 6: EXTRACTED CONSTANTS REPORT
					local constReport = "DEOBFUSCATION / CONSTANT REPORT\n" .. string.rep("=", 30) .. "\n"
					for sPath, data in pairs(extractedData) do
						constReport = constReport .. "\n\128\142 Script: " .. sPath .. "\n"
						if #data.URLs > 0 then constReport = constReport .. "   - URLs: " .. table.concat(data.URLs, ", ") .. "\n" end
						if #data.Strings > 0 then
							local topStrings = {}
							for i=1, math.min(20, #data.Strings) do table.insert(topStrings, data.Strings[i]) end
							constReport = constReport .. "   - Strings (Top 20): " .. table.concat(topStrings, " | ") .. "\n"
						end
					end
					env.writefile(baseDir .. "/_Constants_Report.txt", constReport)
				end)
				
				if #remoteList > 0 then
					pcall(function() env.writefile(baseDir .. "/_Remotes.txt", table.concat(remoteList, "\n")) end)
				end
				
				if #secretFound > 0 then
					pcall(function() env.writefile(baseDir .. "/_Secrets_Audit.txt", table.concat(secretFound, "\n")) end)
				end
				
				print("🎉 [Script Dumper Pro V2] Complete!")
				print("📂 Files generated: _Map_Structure.json & _Constants_Report.txt")
			end)
		end})		--Main.CreateApp({Name = "Secret Service Panel", IconMap = Main.LargeIcons, Icon = "Output", Window = SecretServicePanel.Window})
